...
### Phase 9: Live-surface Taiwan inheritance (third-party wiring) ✅ COMPLETE
...
- [x] **9c. justfont removal.** Removed: SDK script + FOUC fallback + `_jf` config from `Layout.astro` (-237 lines); `.jf-food h1`/`.jf-history h1` category rules + FOUC `html.jf-loading > body` hide + two outside-@layer override blocks from `global.css`; body class simplified (`jfClass` → `bodyClass`, no font class). Shot-mode: `rixingsong-semibold` → `Noto Serif TC` in diary h1, comments updated. `ArticleHero.astro` + `HeadInlineScripts.astro` comments updated. ~40 inline Tailwind `font-['jf-*',...]` declarations across home/semiont components kept (fallback chain works without SDK).
- [x] **9d. Protico removal.** `curl -sI` returned HTTP 404 — endpoint dead. Removed `ProticoScript.astro` component + imports/usages from `home.template.astro`, `about.template.astro`, `contribute.template.astro`. Empty i18n keys (`about.sponsors.protico.*`) left (harmless).

---

**Migration complete (2026-06-25).** Phases 0–9 done. Phase 8 = capability-gated skill backlog (parked, unblocked by ROADMAP.md capabilities). Forward strategy lives in `ROADMAP.md`. Build green (en 0 broken, 0.13% overall). Remaining pre-existing items (6 `/zh-TW` dead links from enabled-but-empty locale) are operational debt, not migration.
