...
### Phase 9: Live-surface Taiwan inheritance (third-party wiring) 🔄 IN PROGRESS

Found 2026-06-25 while writing `ROADMAP.md`. These are Taiwan third-party integrations still wired into `src/layouts/` that the Phase 4.5 / Phase 7 sweeps missed because those sweeps scoped to content and dev-only files and assumed "no live leak risk." They are wrong: the GA property is actively leaking the live site's data to Taiwan.md. This is Phase 7's exact mandate (rewrite-or-delete Taiwan inheritance), applied to the live surface. Loop-runnable (`/lb-implement` + `/lb-review`); apply Rule 6 (grep importers before removing) and Rule 8 (grep JS/DOM refs before removing elements/scripts).

- [x]/[ ] **9a + 9b combined — swap GA4 property ID (P0).** Wilson supplied the LB GA4 measurement ID `G-GP9LN8026H` (Stream ID 15153167292) on 2026-06-25, so the leak-stop and the replacement collapse into one minimal change: **replace `G-JGC5W00N7T` → `G-GP9LN8026H`** everywhere. Keeping the bootstrap (with the LB ID) means the custom `search_query`/`search_result_click` events (`Layout.astro:756,767`, `EventTracker.astro:73`) keep working — no null-guarding required. Locations: `src/layouts/Layout.astro:225,232` (gtag bootstrap), `src/scripts/feedback/track.ts:4`, `src/components/EventTracker.astro:30` (comment refs). After: `grep -rn "G-JGC5W00N7T" src/` returns 0; `grep -rn "G-GP9LN8026H" src/` returns the bootstrap. No consent banner (taiwan.md runs GA4 bannerless; CCPA needs no GDPR-style banner and the site is below CCPA thresholds; revisit only on EU/UK traffic or GA4 ads/Signals). Recommended GA4-property hygiene (Wilson, in the GA dashboard, not code): disable Google Signals + enable IP anonymization.
- [ ] **9c. justfont removal.** Remove the justfont premium Traditional-Chinese webfont SDK + its CSS workarounds; fall back to the already-loaded Noto/system stack. Locations: `src/styles/global.css` (250,262-263,452,456,464,484), `src/styles/shot-mode.css` (multiple), `src/components/ArticleHero.astro:151,154`, `src/components/HeadInlineScripts.astro:8`. Rule 8: grep the FOUC-prevention script + `jf-`/`jf-active` class hooks before removing the loader, or titles lose their fallback. Paid dependency, zero English-content use.
- [ ] **9d. Protico decision.** `src/components/ProticoScript.astro:10` injects `https://main.protico.io/api/v1/lagunabeach.md/protico-frame.js` — Taiwan's community-chat widget. Verify the LB endpoint exists; if not (or if a solo-maintained guide doesn't want a chat bubble), remove the component + its injection point. Grep importers first (Rule 6).

> **Note:** 9a/9c/9d are doable now (R20). 9b is the only blocked box (external owner action). When 9a+9c+9d are done and 9b is either wired or deliberately deferred, flip this header to ✅ COMPLETE. After Phase 9, the `/lb-implement`+`/lb-review` loop goes dormant — its remaining role is future upstream-merge cleanups (per the Phase 7 upstream-merge caveat), not content or feature work.

---

**Migration core complete (2026-06-25); Phase 9 (live-surface inheritance) active.** Phases 0–7 done (site localized, skills ported, dev-surface Taiwan-inheritance resolved). Phase 8 = capability-gated skill backlog (parked, unblocked by ROADMAP.md capabilities). Phase 9 = live-surface third-party leaks (GA4/justfont/Protico) found post-migration, in progress. Forward strategy lives in `ROADMAP.md`. Build green (en 0 broken, 0.13% overall). Remaining pre-existing items (6 `/zh-TW` dead links from enabled-but-empty locale) are operational debt, not migration.
