...
**Approved 2026-07-07; consolidated 2026-07-26.** This is the delivery-plan SSOT.
Milestones are Linear project milestones on "LB Rebuild", one per phase. Task packets
...
with the same contract as phases 6-8.
...
| 1 | Core pages at design parity | Layout, home, article, category hub, explore/search, latest, about/contribute/changelog, `/kb/` + llms.txt, prebuild chain | 1.1a-c · 1.2a-c · 1.3 · 1.4 | **Wilson design sign-off on 1.1c side-by-side package** (SPEC `Risk controls`); phase confirm | AI 19.5h \| Human 1h |
...
Completed phases 0-5 are summarized above. Linear retains their final task packets,
work summaries, review findings, and verification evidence. Their superseded pre-execution
blocks and the retired `.handoff` loop are intentionally not duplicated in this operative
roadmap; they remain recoverable from git history.
...
support is a post-project revisit after Phase 11 (PRD non-goals). `/dev:plan` must not
emit packets that retain CJK code for
...
README, framework AGENTS.md, playbook) state the support boundary explicitly rather than
...
the framework ships before LB's fun features (ADR 002; SPEC `Risk controls`). Phase 9
depends on 7.2c (the inherited-fork disposition's named MCP trigger); Phase 11 depends on 8.1 (ROUTINE organ architecture)
...
- **8.1 organ-loader ruling (supersedes task 8.1 step 2's "CLAUDE.md boot section" and
  ADR 003's original loader location):** the loader is a stable one-paragraph boot hook seeded into
...
- **8.2 naming:** the acceptance's "lb-become boots" reads as the 8.1 boot hook (the
  `lb-become` skill was re-dispositioned into 8.1 by the 2026-07-11 ruling).
...
[6.1a] Feedback backend: workers/feedback (Cloudflare Worker + D1)
  Effort: M | Model: Opus | Depends: 5.4
...
[6.2] Snippet pipeline: skill + inbox + adapter interface
  Effort: M | Model: Sonnet | Depends: 5.4
  Est: AI 2h + 0.5h review
  Steps:
    ...
    2. Define the platform-adapter interface. Add no platform adapter until an LB account
       exists; then add one adapter per platform.
...
[6.3] Soundscape page + first three recordings
  ...
  Steps:
    ...
    3. Wilson supplies the first three recordings.
...
[7.2a] Corpus embeddings: build-embeddings.mjs + static vectors
  ...
  Mandatory pre-read: v0 research section "RAG Chatbot"
  Steps:
    1. Port build-embeddings.mjs from the v1 archive. Chunk articles at 300-500 tokens on
       ## headings; embed with bge-m3 at 1024 dimensions on the 4090 or Workers AI. Emit
       static vectors JSON into workers/chat/.
...
[7.3] QR flow: location-context deep links + printable codes
  ...
  Acceptance: scan to /chat?ctx=main-beach opens with a location-aware greeting
  Downstream: none
```
...
[8.2] LB enables core + MANIFESTO; DIARY and ROUTINE stay off
  Effort: S | Model: Sonnet | Depends: 8.1
  Est: AI 1h + 0.25h review
  Steps:
    1. Enable memory, reflexes, and manifesto in LB's semiont/config.json.
...
Model policy: all execution Opus (Wilson,
...
[9.2] AI-access page + agent boot file
  ...
  Est: AI 1h + 0.25h review
  Steps:
    1. /ai page (successor to the inherited-fork MCP page) documenting every AI consumption path
...
[10.2] Signal fetchers + dashboard analytics panels
  ...
  Steps:
    ...
    2. Dashboard gains traffic/search panels (Chart.js per SPEC `Stack` only if needed); build stays
...
[11.2] Embeddings + index refresh pipeline (CI-triggered, deterministic)
  ...
  Steps:
    1. GH Actions job on push-to-main touching knowledge/**: rebuild chunk vectors via
       ...
       path stays documented as the offline alternative per SPEC `Stack`), redeploy the vectors
