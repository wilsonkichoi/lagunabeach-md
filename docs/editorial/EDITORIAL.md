...
3. **iframe + image density ≤ 0.8 / 1k CJK chars**（觀察者 directive override 寬限 ≤ 1.0/1k）— Visual 是輔助呼吸不是替代敘事節奏
...
- 找不到官方 / 高品質影片來源 → 不勉強塞 iframe，多放 1 張圖補位（影片不是 KPI）
- 文章超過 5000 字 → 上限可放寬到 4 圖 + 5 影片，避免讀者疲勞
- 翻譯文 → 跟原文同步（不另加 / 不另減）
