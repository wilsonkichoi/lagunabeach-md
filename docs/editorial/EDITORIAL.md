...
用「希望台灣變得更好」的眼光寫，但盼望蓋在誠實上。改的是看的姿態跟讀者帶走的情緒，不是挑好的事實寫。報喜不報憂 = 粉飾太平 = 策展崩解（§七 紀實而不煽情 的另一半）。

**判準句**：讀者讀完這段，是更想為台灣做點什麼（盼望 + 能動性），還是更焦慮、更無力、更覺得自己不夠好（PUA + 媒體焦慮）？前者留，後者改。
...
`prose-health` plugin 自動抓 PUA 體 + 媒體焦慮體句法（WARN 級，逐處由人判斷是陳述還是販賣焦慮）。

### 自稱台灣，不是「這座島」

完整哲學見 [MANIFESTO §跟台灣的關係 §自稱：台灣，不是「這座島」](../semiont/MANIFESTO.md)。

**不要的**：用「這座島」「這個島」「這座島嶼」「這個地方」當台灣的迴避稱呼。這是國際媒體與 PRC 框架的指紋（避免稱台灣為國家），模型會預設吐出來。Taiwan.md 的公理是「台灣是主權獨立的國家」，行文自我降格成「島」是拆自己的地基。

**要的**：大方寫「台灣」「臺灣」「這個國家」。

**例外**：曹永和「以島嶼為主體」的島史脈絡，島嶼是歷史主角（主權肯定），可寫。

**自檢**：`prose-health` plugin 自動抓「島嶼自稱 crutch 密度」（ratio-based：島佔「島 + 台灣」國名指稱 > 1/4 且 ≥ 3 次，或完全不稱台灣才 WARN；單次文學用法不罰）。WARN 跳出時逐處判斷：曹永和式主體島史（留），還是不敢寫「台灣」的迴避（換）。
