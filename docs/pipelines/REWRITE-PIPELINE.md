...
# REWRITE-PIPELINE.md — 文章改寫主流程 v6.3
...
| **1 研究深挖**   | N 個 parallel sub-agent（按子領域切：電影/電視/遊戲/歷史…） | **Sonnet**（breadth + extract；contested atom escalate Opus） | falsification-first，結構化 verification table 落報告                                                    | 各自獨立回報結構化 findings；orchestrator 合成去重成 clean fact-pack                                                 |
