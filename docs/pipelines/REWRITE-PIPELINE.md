...
# REWRITE-PIPELINE.md — 文章改寫主流程 v6.4
...
| **研究報告 SSOT health** 🔬 | Stage 1 終 | **所有 depth**                | `research-report-health.py --tier=depth`（distinct≥25 / en≠0 / 一手≠0 / 搜尋日誌 / raw §7） | **不進 Stage 2** |
...
3. **Step 1.7 研究報告 = SSOT** — 搜了沒把原始軌跡寫回 §7 = 沒搜（v6.4，TDRI session 退化教訓）；跑 `research-report-health.py` 驗收
...
#### 1.7.1 SSOT 七段結構（depth article 強制）
...
全部 query（Stage 0 + Stage 1）逐條：`query → 一句話發現 → [source](URL)`，
每條標 [中]/[英]/[一手]/[學術]/[反方]。這是 thesis 的 methodology section。

## 3. Findings by sub-topic（§A / §B / §C …）

每個子題分章，每個 claim 後標來源 + 信度（高信度 / 單一來源警示 / 必驗 / 未驗證），
引語標 verbatim + Ctrl-F 結果。對標 gold standard 毒馬鈴薯認知作戰.md 的 §1-§N。

## 4. Clean Fact-Pack（給 writer 的合成層，額外、不取代上面）

去重後的乾淨事實 + 幻覺護欄 + 媒體 manifest。Stage 2 writer 只吃這層。

## 5. 參考文獻（Bibliography）

全部 distinct 來源列表，標 [中]/[英]/[一手]/[學術]。對標論文參考文獻。

## 6. Verification Table（高風險 atom）

| claim | sources | Ctrl-F | 信度 | verdict |

## 7. Agent 原始輸出（raw，不摘要，append 全部）
...
2. 主 session 把**所有 agent 原始輸出 verbatim append 到 §7**（SSOT raw）。
3. 主 session **額外**合成 §4 Clean Fact-Pack 給 writer。
4. §4 是 §7 的蒸餾，不是 §7 的替代。**§7 缺席 = Stage 1 未完成**。
...
- **+ SSOT**：reader callout 質疑某 claim → 直接在 §6 Verification Table + §7 raw 追到當時逐字來源，不用重搜

**存檔責任**：Stage 1 主 session 在 agent 回傳後**同一個 response** 內寫 §1-§7 完整檔 + 跑 research-report-health gate，不 defer。raw §7 缺席或 gate hard_fail = Stage 1 未完成。

**讀取責任**：Stage 2 Write 開始前，grep `reports/research/` 看有無相關主題報告可 cross-reference。**Writer agent 只吃 §4 Clean Fact-Pack**（context 隔離，per §多 agent 編排）。
