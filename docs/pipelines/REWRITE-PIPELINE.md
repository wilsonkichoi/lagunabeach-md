...
| **2 寫正文**               | 1 個 **fresh** sub-agent                                   | **Opus**                                                      | 寫作 craft 最高判斷；fresh context 才乾淨                                                                                                   | **明確要求 writer 先 Read 整份 research report（§6 fact-pack ＋ §8 raw verbatim 全部）+ §觀點 + EDITORIAL + pipeline**；隔離的是**舊文 prose / callout / orchestrator 累積 context**，不是 report。⚠️ **禁止只貼 orchestrator 摘要的精簡 fact-pack 又叫 writer 別讀 report**（摘要漏 raw texture → 文章變爛，2026-06-15 哲宇 callout） |
...
- **首選**：spawn 一個 fresh writer agent（Step 1.8 既有 spawn 機制），prompt **明確要求它先 `Read` 完整 research report 檔案**（不是只貼 orchestrator 摘要的 fact-pack）＋ §觀點 + anchors，**不給舊文 body、不給 callout**。Agent 在乾淨 context 裡像第一次寫，但握有研究的全部 raw texture。⚠️ **反 pattern（v7.4，2026-06-15 哲宇 callout）：orchestrator 把 report 再摘要成精簡 fact-pack 塞進 prompt、又叫 writer 別讀 report ＝ 雙重失真，近期文章變爛的根因。**
