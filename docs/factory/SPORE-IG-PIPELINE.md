...
**`chart-line`（← graph.md `tw-line`）** — 趨勢折線（inline SVG，自動 y 軸 + 終點直接標籤）。欄位 `{ title?, x:[...], series:[{name,points:[...]}], source_note? }`。**多序列同尺度**。何時用：≥ 3 時間點趨勢 / 同尺度多序列對照（房價所得比 vs 世銀警戒線）。**禁止**：雙 Y 軸（不同尺度兩指標 → 拆兩張）/ 截斷 y 軸誇大。
