...
description: 'IG 圖文孢子產線 — 4 階段 PICK → CURATE → DRAFT → RENDER（+ SHIP / HARVEST archived future）。物種紀律 + 母片系統 + 字數 floor/ceiling + 單一識別色。v0.8：Stage 4 加「人眼層」自檢 gate（機械全綠≠過人眼）+ code 語法高亮 + 圖表 honesty（waffle 三 tint 分離 / line ref 虛線基準 / heatmap 二次對比）+ cover 副標 em。v0.7：cover date + section code/截圖。v0.6：graph.md 7 圖表母片全落地。v0.9：頁數回 10 標準（IG 電腦版擋 >10）。'
...
>
> 設計理由：文字 SPORE 跑在 Threads/X，拼觸及與單一弧線；IG 圖文跑的是**收藏 / 分享 / 跨群轉傳**。同樣的素材，要重新策展、不是搬運。SPORE 的「150-300 字單一故事」放不下文章的多層次敘事；IG carousel 一次 10 頁，剛好是「**把長文濃縮成 10 個 micro-scene**」的尺寸。
>
...
**動作**：**10 頁標準** carousel（哲宇 2026-06-07：IG 電腦版上傳擋 >10，10 是安全上限；不足 10 = 浪費 algorithm exposure，素材太厚 → 砍到最強 10 張），每頁列一行：
...
> **10 張標準**（哲宇 2026-06-07：IG 電腦版上傳擋 >10，10 是安全上限；部分手機 app 雖到 20 但不可靠，先以 10 為準）。低於 10 = 浪費 algorithmic exposure（IG 把每張 slide 當獨立 engagement event）。素材太厚 → 砍到最強 10 張；要展全母片的 dev showcase（如視覺化母片型錄 15 張）當內部基準、不上 IG。
...
| **v0.9**   | 哲宇「先限制 10 張為標準，電腦版會擋」                                                      | **頁數政策 10-20 回退 → 10 張標準**（IG 電腦版上傳擋 >10，10 是安全上限；手機 app 雖到 20 但不可靠）。ship-deck 一律 10；要展全母片的 dev showcase（型錄 15 張）當內部基準不上 IG。週報 carousel 從 19 砍回最強 10 張 + 配 ~1000 字 caption（報告精華版）                                                                                                                                                                                                                     |
