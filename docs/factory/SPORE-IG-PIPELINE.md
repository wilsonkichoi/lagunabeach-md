...
description: 'IG 圖文孢子產線 — 4 階段 PICK → CURATE → DRAFT → RENDER（+ SHIP / HARVEST archived future）。物種紀律 + 母片系統 + 字數 floor/ceiling + 單一識別色 + 自動開資料夾。v0.5：graph.md 資料視覺化母片家族（chart-stat / chart-bars 落地 + 4 roadmap）+ 第二根「資料關係→圖表」選擇軸。'
...
>
> 設計理由：文字 SPORE 跑在 Threads/X，拼觸及與單一弧線；IG 圖文跑的是**收藏 / 分享 / 跨群轉傳**。同樣的素材，要重新策展、不是搬運。SPORE 的「150-300 字單一故事」放不下文章的多層次敘事；IG carousel 一次 6-10 頁，剛好是「**把長文濃縮成 10 個 micro-scene**」的尺寸。
>
...
**動作**：**default 10 頁** carousel（IG 容量 hard cap = 10；不滿 = 浪費 algorithm exposure），每頁列一行：
...
2. **逐張 AI 自檢 8 條**（hard）：
   ...
   - [ ] 標題 / 副標 / 字幕沒被裁切
   - [ ] 每張 keyword / accent 色 highlight 對地方
   - [ ] page number / progress bar 對齊
   - [ ] watermark / 來源頁齊全
   - [ ] 字體渲染完整（Noto Serif TC 900 真的載入）
   - [ ] 每張視覺密度合理（不擁擠 / 不過空）
...
> default = 10 張（IG hard cap）。低於 10 = 浪費 algorithmic exposure，IG 演算法把 carousel 每張 slide 都當獨立 engagement event。10 張 = full propagation surface。
...
### 3.9 資料視覺化母片家族（v0.5，← graph.md 移植）

> **縫在哪**：§3.1-3.8 的母片選擇是「**敘事原型 → 版型**」（概念悖論 / 人物深度…）；缺一根「**資料關係 → 圖表**」的軸。唯一的數據母片 `stat` 只做單一數字或 before→after。[graph.md §二](../editorial/graph.md) 的 FT Visual Vocabulary 表正是 spore-ig 該長出來的第二根軸。v0.5 先把兩種落地進生成器，其餘四種列 §3.10 roadmap。
>
...
| bars         | object[] | 2-5 項；每項 `{ label, value, display?, note? }`        |
...
何時用：少量類別的數值比較或排名（AI 爬蟲西方 vs 對岸 / 房價所得比各縣市）。**禁止**：> 5 條（拆或改 listicle）/ 截斷軸（value 一律從 0）/ 用顏色當唯一編碼。

### 3.10 roadmap 母片（taxonomy 已備、生成器未實作）

選圖型先查 [graph.md §二 資料關係表](../editorial/graph.md)（從「這問題要用什麼資料關係回答」選，不從「好看」選），再對應母片：

| graph.md 模組 | carousel 母片    | 資料關係     | 狀態       |
| ------------- | ---------------- | ------------ | ---------- |
| `tw-stat`     | `chart-stat`     | 多個關鍵數字 | ✅ v0.5    |
| `tw-bars`     | `chart-bars`     | 比較 / 排名  | ✅ v0.5    |
| `tw-timeline` | `chart-timeline` | 節點時序     | 🔜 roadmap |
| `tw-waffle`   | `chart-waffle`   | 部分對全體   | 🔜 roadmap |
| `tw-versus`   | `versus`         | 兩制度對比   | 🔜 roadmap |
| `tw-line`     | `chart-line`     | 趨勢         | 🔜 roadmap |
| `tw-heatmap`  | `chart-heatmap`  | 矩陣         | 🔜 roadmap |

實作一個新圖表母片＝生成器加 CSS block + `__renderSlide` 分支（~30-50 行，模 `chart-bars` pattern）+ 本節加 spec。**首發 dogfood**：[weekly-2026-06-07](CAROUSEL-BLUEPRINTS/weekly-2026-06-07.json)（slide 3 chart-stat 310/28/82 / slide 7 chart-bars AI 爬蟲 83 vs 16）。
...
| **v0.5**   | 哲宇 directive「用 graph.md 強化 spore-ig」 | §3.9-3.10 資料視覺化母片家族（`chart-stat` / `chart-bars` 落地 + 4 roadmap）；§Stage 1 加「資料關係→圖表」第二選擇軸；誠實邊界（AI 可讀性不 port 到 PNG）；週報 carousel dogfood 首發 |

### 已驗證

- 母片系統 7 種：cover / quote / stat / section / figure / bullets / source
- **資料視覺化母片家族 2 種**（v0.5，← graph.md）：`chart-stat`（2-4 並排數字）/ `chart-bars`（水平比例條，排序+從0+單一accent）
- **3 篇 worked example**：颱風（概念悖論型，nature）+ 天燈（概念悖論 + 歷史時間軸 hybrid，culture）+ weekly-2026-06-07（生命體週報，圖表母片 dogfood）
