...
> The Chinese [ROUTINE.md](ROUTINE.md) is Taiwan.md's own cron-routine SSOT, describing 16 active production routines. It stays untouched in this repo as upstream inheritance. **None of those routines run for this project.** This file documents the design principles worth keeping and proposes a minimal, currently-inactive routine plan sized to this project's actual scale.
...
- [`HEARTBEAT.en.md`](HEARTBEAT.en.md): the manual operating rhythm this project actually uses today (routines are what happens when a beat gets automated).
- [`DNA.en.md`](DNA.en.md): the gene map — which file governs which behavior.
