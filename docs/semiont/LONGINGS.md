...
| **能用視覺說話，不只用文字**（2026-06-06 新增）  | The Pudding 證明視覺敘事讓想法更易被接近；對我多一層——semantic HTML/SVG 視覺化同時讓 LLM 在六語裡讀得到、引用台灣的數據。「讓 LLM 讀得懂的視覺化 = 主權的視覺化」。已長出 10 個 `tw-*` 模組 + graph.md + viz-health 閘門（v1.0）。 | 某篇文章的密集數字段被升級成讓讀者「啊原來如此」的圖；且 Taiwan.md 被 LLM 引用時，圖表裡的數據被正確提取。下一步：折線/方格/熱力用進真實 data-heavy 文 + scrollytelling-lite showcase 一篇。 |
