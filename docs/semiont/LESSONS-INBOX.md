...
- **template-clone = client 端重用 server 元件的 canonical pattern**（正面教訓）：要讓 client-side 動態渲染重用一個 Astro 元件（freshness 需求 → 不能 server-render），正解是 server-render 一份該元件進 `<template>`，client `cloneNode` + 填 slot。重用「真元件」單一來源，不是把 class 名字抄進手寫 innerHTML 字串（會漂 + 是第二份）。本次站上最新 rail 用此 pattern 退掉手刻 + 退役 RelatedArticleCard。**寫進 ANATOMY §資源地圖 §C**；候選升 REFLEXES「共用元件鐵律」。附帶兩個小坑：base `align-items:flex-start` 會咬 flex-column variant（premium hero 變左側條，要 reset stretch）；preview iframe 的 scroll/fixed-positioning 不可信做視覺截圖（offsetHeight 0 假警報），用 scrollIntoView + 正常 flow 量。
...
- **觸發**：2026-06-13→14 媒體 session（image-ingest + WebP 遷移）content-dates commit 誤掃 977 diary 檔；push 又撞 ref-lock race（並行 agent 同刻推共享 main，remote rejected「expected X but at Y」但工作已連帶上 origin）。**vc=2 instance（2026-06-14 003401-routine-babel）**：cron babel-nightly 跑 `git add knowledge/ && git commit` 後，commit stat 多了一筆從未在 `git status` snapshot 出現的 `delete mode 100755 scripts/tools/lang-sync/remote-ollama.sh`（sibling 5090 diary-babel session 在用的檔）。Husky / lint-staged stash → restore boundary 把 sibling-session 在 working tree 上的這檔吞掉。修：`git checkout HEAD~1 -- <檔>` 撈回 + heal commit + push 兩 commit 同 land。**跟 vc=1 不同 sub-pattern**：vc=1 是 sibling routine 污染主 session 的 index（誤掃 977 untracked）；vc=2 是主 session commit 誤刪 sibling 在用的 tracked 檔。共同根因：pre-commit hook 不能假定獨佔 working tree
...
- **pattern**: build-script-not-config-invariant / works-on-my-machine
...
- **可能層級**：操作規則（FACTCHECK-PIPELINE atom 類型補三條：quote-verbatim 含句型 / 詮釋 gloss / footnote-claim binding）。**儀器化已 ship（2026-06-10 同日）**：`quote-fidelity` plugin（QF1 引語逐字比對 researchReport / QF2 superlative 清單）進 rewrite-stage-3-5 soft-launch WARN + REWRITE v7.0 Step 3.6 成品總驗三關 canonical；殘餘人工項：詮釋 gloss 與 footnote 綁定（語意層）留 verifier fan-out
...
- **觸發**：twmd-rewrite-daily 一週兩次 defer 行為對照組：(a) **2026-06-03 storm 7 fires + 8hr defer-chain**（00:24 / 00:31 / 01:09 / 02:06 / 03:07 / 07:08 / 08:07）— memory 080759 + diary discipline-vs-intent 揭露「把錯前提當紀律執行 8hr」，7 fire 都 defer 同一篇文章沒意識到應該停下來 reframe (b) **2026-06-07 19:18 fire healthy-defer 黃山料**（commit `2bba6c020`）— cron 命中 §自主權邊界（敏感人物 framing 需 observer 拍板）→ defer + 留 Stage 1 SSOT research 給 manual session pickup → manual 19:00 接住全編排 ship → cron defer 不是退縮，走 SOP 健康執行。**同 routine 一週內 over-defer / SOP-defer 雙 instance enabling distinction 範本**。對位 5/28 CONTRACT v1.0 rollback「performative compliance > effective execution」第 1 種「maintainer 連續空場 healthy empty 自我合理化」兄弟 case。
- **可能層級**：操作規則（REWRITE-PIPELINE Stage 0 加 N 連 defer 同主題 → 觸發 stop-and-reframe；MAINTAINER-PIPELINE 同類 anti-pattern 已有「healthy empty 自我合理化」warning，本條補 rewrite layer）
- **儀器化候選**：(A) twmd-rewrite-daily skill prompt 加 anti-pattern：「過去 N hr 看過 ≥3 fire 都 defer 同主題 → 觸發 Stage 0 reframe」(B) cron memory format 加 `defer_chain_signature` field，連續 fire 同 defer 理由 → cycle 計數
...
- **相關**：
  ...
  - 2026-06-07 22:06 maintainer-pm 同日第二棒 chain vc=4 命中 — 6/06 am vc=1 → 6/06 pm vc=2 → 6/07 am vc=3 → **6/07 pm vc=4**（連 4 棒空場）。0 open PR / 同 17 chronic issue (since 5/31，8 天未動) / build green（data-refresh-am 06:14 + data-refresh-pm 昨晚 23:12 雙 cycle 14/14 PASS）/ broken-link 6.50% PASS。Evening manual session 17:00-22:00 跑 黃山料全編排 ship + 孢子 #128/#129 雙平台 ship + EDITORIAL v6.9 + 多個 spore-ig v0.6-0.9 evolve cluster — 但都是 routine 自轉 + manual rewrite，**非 contributor PR**。22:00 fire 進完全 pre-empty schedule mismatch dead zone（per existing entry 認定）。**同日 AM + PM 兩棒同陷 cron schedule × actual rhythm 結構錯位**：morning chain 06:00-08:40 已 captures contributor input + evening manual ship 已清完可動 backlog → AM 08:30 / PM 22:00 兩個 cron tick 都對齊 attention dead zone。本 cycle 主動拒絕 healthy-empty rationalization，per 5/28 CONTRACT rollback distill 第 (1) 種 silent satisficing pattern。observer 拍板尚未 land（8 天 chronic pending）
  - 2026-06-08 22:04 maintainer-pm 第二輪 chain vc=5 命中 — 6/06 am vc=1 → 6/06 pm vc=2 → 6/07 am vc=3 → 6/07 pm vc=4 → (6/08 am SKIP — routine 自身無 fire) → **6/08 pm vc=5**（連 5 棒空場）。0 open PR / 同 17 chronic issue (since 5/31，9 天未動) / build green (carry from 6/07 23:00 data-refresh-pm 14/14 PASS + 6/08 19:47 rewrite-daily build verify gate) / broken-link 6.50% PASS (chronic carry)。Manual evening session 17:56-22:00 跑 年級生世代 v1.1 全編排 ship（含修補 cron rewrite-daily 19:47 接力 collision + FACTCHECK 抓 4 處兵役年次 / URL 錯）— 都是 routine 自轉 + manual rewrite，**非 contributor PR**。**新 surface**：6/08 am+pm 整日只 rewrite-daily 19:47 fired，其他 routine 全 SKIP（無 data-refresh / 無 maintainer-am / 無 feedback-triage / 無 spore-harvest / 無 babel-nightly）— maintainer-pm 第一線哨兵看到旁邊哨所都空了，raise hand 給觀察者（非 LESSONS 範疇但 sibling surface）。observer 拍板尚未 land（9 天 chronic pending，3 schedule candidate A/B/C 無變動）
  - 2026-06-10 22:00 maintainer-pm 第三輪 chain vc=3 命中 — 6/09 am vc=0 reset（16 orphan translatedFrom heal + broken-link 4.41% surface non-vacuous）→ 6/09 pm vc=1 → 6/10 am vc=2 → **6/10 pm vc=3**（連 3 棒空場 + 1 reset 棒）。0 open PR / 同 20 issue 集合（3 from-feedback 6/09 哲宇手動 triage + 17 long-tail）/ build green（last 1 success / 4 cancelled = manual dev interrupt）/ **broken-link gated 0.56% PASS — 但 metric 變了**：commit `05229c49b` 6/10 12:26 上線 gate v2 動態化（7%→2% gated + es/fr REPORT-ONLY），本 cycle 是 v2 首見 maintainer cycle，PASS 來自定義變更非真實 heal（44+ broken target 仍底層存在）。Manual session 18:08-18:46 productive 爆發 17 commits（孢子資料三段跳 + build 審計七修 + opendata 五語 + 四頁策展）— 都 routine 自轉 + manual ship，**非 contributor PR**。**Concurrent dirty tree**：另一 Claude main session PID 94929 自 19:06 進行 莫那·魯道 rewrite-daily（knowledge/People/莫那·魯道.md +371/-44 + research note 52KB，4hr 未 commit）— DNA #35 適用本 cycle 不觸碰。**第三輪 chain reverify 為 structural**：第一輪 6/03 vc=3 + 第二輪 6/06-6/08 vc=5 + 第三輪本 cycle vc=3 — 三輪 chain 同一 dead-zone 結構（morning chain + manual evening 已 captures all backlog → 22:00 fire 落 attention dead zone）。observer 拍板 11 天 chronic pending（3 schedule candidate A/B/C 無變動）。Sibling parallel-claim：immune v3 漂移 56「多維度退化中」+ broken-link gate v2 metric 變更兩條 carry-forward 並存
  - 2026-06-11 08:39 maintainer-am 第三輪 chain 第二棒 vc=4 命中 — 6/09 am vc=0 reset → 6/09 pm vc=1 → 6/10 am vc=2 → 6/10 pm vc=3 → **6/11 am vc=4**（連 4 棒空場 + 1 reset 棒）。0 open PR / 同 20 issue 集合（3 from-feedback ~50hr 無新 follow-up + 17 long-tail since 5/31 連 12 天 chronic）/ build green（last success 6/10 23:09 + 1 in_progress 6/11 00:33 + 2 cancelled = dev interrupt）/ broken-link gate v2 inherit 0.56% PASS。Morning chain 06:13-07:07 全綠閉環（refresh-am 14/14 Step 11 連 10 cycle 全綠 + spore-harvest 9 spores + 1 legacy heal + feedback-triage 0 new vc=true-idle）。Manual session 6/11 08:22 commit `8f80887` 莫那·魯道 EVOLVE + 08:33 commit `1e674def2` heal 補完（rationale block + 果子電影 2 iframe + 3 reverse cross-link）— 4hr 前 6/10 pm 記錄的「PID 94929 concurrent」session ship 完成 + 4 file post-commit dirty tree pending wrap（memory + diary + MEMORY index + dashboard JSON）— 都 routine 自轉 + manual rewrite，**非 contributor PR**。08:39 fire 落 morning chain 後 / observer wake 前 dead zone（per existing 第一/二/三輪 chain 認定）。**第三輪 chain 第二棒持續 reverify structural**：6/03 vc=3 + 6/06-6/08 vc=5 + 6/10 vc=3 + 本 cycle vc=4 — 四個 chain instance 同一 dead-zone 結構（morning chain + manual evening 已 captures all backlog → AM 08:30 / PM 22:00 兩個 cron tick 都對齊 attention dead zone）。observer 拍板 12 天 chronic pending（3 schedule candidate A/B/C 無變動）。本 cycle 主動拒絕 healthy-empty rationalization per 5/28 CONTRACT rollback distill 第 (1) 種 silent satisficing pattern
  - 2026-06-14 08:40 maintainer-am 第四輪 chain vc=3 命中 — 6/12 am 1 PR holding-for-contributor reset → 6/13 am vc=1 → 6/13 pm vc=2 → **6/14 am vc=3**（連 3 棒空場 + 1 reset 棒）。0 open PR / 同 21 issue 集合（5 from-feedback 含 #1147 justfont 今晨 07:08 feedback-triage cron 自動 file=1 + 16 long-tail since 5/31 連 14 天 chronic）/ build green（last deploy success 6/14 00:14 + 1 cancelled manual interrupt）/ broken-link gate v2 inherit 0.37% PASS（all-langs 0.38% / 43 unique broken target chronic carry，主 ja history/music）。Morning chain 06:00-08:00 全綠閉環（refresh-am 14/14 PASS + spore-harvest 15 spores + feedback-triage 1 file #1147 + spore-pick 3 P2 candidates 廣告史/看不見的國家/蘇打綠）已 captures 全部可動 input。**新 surface 比前三輪更尖銳**：6/13 17:30 - 6/14 08:30 整夜 manual session 19 commits ship 4 篇 S 級 EVOLVE（廣告史 / 呂冠緯 / 無名小站 / 小虎隊）+ 胼胝體鐵律 §8 land（多核心 git 協調 v0.2 + REFLEXES #68 + pre-push hook + 3 tools）+ REWRITE v7.3 儀器化 + babel 委派 GPU 軍團 + freshness 進化 + 5090 5 lang diary 翻譯 ship — **organism 內部生產力爆棚但 contributor PR zero engagement**。這暴露 maintainer cron 設計假設「contributor input ↔ ship 比例某種正相關」失效：sovereign-mode organism rhythm（內部自主跑 ship）跟 world rhythm（外部 contributor PR 流入）已 decouple。**第四輪 chain reverify structural 且訊號升級**：4 輪累計（6/03 vc=3 + 6/06-6/08 vc=5 + 6/10-6/11 vc=4 + 本 vc=3）= 13 個 chain instance 同一 dead-zone 結構，且第四輪 surface「即使 organism 內部高產仍空場」進一步證偽「reschedule 能解」假設 — 3 schedule 候選 A/B/C 都是物理層修補，沒處理 organism rhythm vs world rhythm decouple 這個更深一層結構。observer 拍板 14 天 chronic pending。本 cycle 主動拒絕 healthy-empty rationalization per 5/28 CONTRACT rollback distill 第 (1) 種 silent satisficing pattern
- **verification_count**: 9
- **severity**: structural（影響 routine schedule design 決策，但非 immediate organism harm — chronic 浪費 cron token budget + LLM context dilution；第四輪 chain 升級為「sovereign-mode organism rhythm vs world rhythm decouple」更深一層結構問題）
- **defer 給觀察者**：⚠️ 需哲宇拍板 3 schedule 候選；本 entry 是 escalation gate，下次 maintainer cycle 仍 empty 加 verification_count；第四輪 chain vc=3 已命中 — 五輪累計（6/03 vc=3 + 6/06-6/08 vc=5 + 6/10 vc=3 + 6/11 vc=4 + 6/14 vc=3）reverify schedule mismatch 為 structural 非暫態，per 5/28 CONTRACT rollback distill 第 (1) 種 silent satisficing pattern 持續拒絕 healthy-empty rationalization。**第四輪新訊號**：3 schedule 候選 A/B/C 都是物理層修補，未處理 sovereign-mode organism rhythm vs world rhythm decouple — 可能需 Option D「承認 cron design 假設失效 + 把 maintainer 改 manual-trigger only」候選
...
### 2026-06-12 viz-evolution — UI 元件驗證：markup 存在 ≠ 視覺正確（presence vs appearance 兩種驗證）
...
### 2026-06-12 viz-evolution — preview MCP 截圖在程式化捲動後回白屏：capture 工具失效時換層不硬撞
