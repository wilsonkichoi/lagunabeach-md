---
title: 'ROUTINE'
description: 'Routine 飛輪 SSOT — 14 條 active TWMD-prefix cron routine 排程、quality gate、escalation（含週日反思鏈 4 條 + maintainer 1d2x 守 PR backlog + spore harvest 06:30 Chrome MCP audience flywheel + spore pick 08:00 intake + spore publish 17:30 evening output + routine-audit-weekly 飛輪自審 21:00 Sunday）。v2.7（2026-05-27）：4 routine time-shift 避開哲宇 9am-5pm 工作時段 + spore-harvest 升 audience flywheel cycle'
...
| `twmd-babel-nightly`        | TWMD babel (nightly) ³       | `30 0 * * *`       | `/twmd-babel`           | Opus   | 每天 00:30     |
...
Slot 釋出給 `twmd-spore-publish-daily`。Skill + script + audit data 保留，仍可 `/twmd-music-media-audit` manual 跑。
...
│ 23h  │  r  r  r  r  r  r  r      │
│ 00h  │  B  B  B  B  B  B  B      │  ←  夜間 chain 收尾 (babel push stale→0, v2.8 shift 05:00→00:30 修補 morning chain collision)
│ 01h  │  ·  ·  ·  ·  ·  ·  N      │  ←  週日反思鏈 (start)
│ 02h  │  ·  ·  ·  ·  ·  ·  W      │
│ 03h  │  ·  ·  ·  ·  ·  ·  D      │
│ 04h  │  ·  ·  ·  ·  ·  ·  E      │
│ 05h  │  ·  ·  ·  ·  ·  ·  ·      │  ←  (babel v2.8 已從 05:00 搬到 00:30 → 06:00 morning chain 不撞)
├──────┼───────────────────────────┤
│ 06h  │  a  a  a  a  a  a  a      │  ←  白天 morning chain start (a 06:00 + S spore-harvest 06:30)
│ 07h  │  F  F  F  F  F  F  F      │  ←  feedback-triage → from-feedback issue (v⁹ 2026-06-01)
│ 08h  │  P  P  P  P  P  P  P      │  ←  spore-pick daily intake (v2.5)
│ 09h  │  M  M  M  M  M  M  M      │  ←  白天 morning chain mid
│ 10h  │  X  X  X  X  X  X  X      │  ←  spore-publish (v2.6 — output layer)
│ 11h  │  ·  ·  ·  ·  ·  ·  ·      │  ╮
...
Legend:
  ...
  S = twmd-spore-harvest-am      (opus, Chrome MCP harvest D+1-D+7 OVERDUE — fires 06:30)
  F = twmd-feedback-triage       (sonnet, 讀者回報 → from-feedback GitHub issue — v⁹ gated on Supabase)
  ...
  X = twmd-spore-publish-daily   (opus, SPORE-INBOX 挑 1 條過 4 hard gate → 自動 ship 雙平台 + 復盤 — v2.6)
  A = twmd-routine-audit-weekly  (Sun, opus, 飛輪自審 7-day pattern detection — v2.4 noon)
  · = idle (no routine fire)
  DISABLED 2026-05-25: twmd-music-media-audit-weekly (was Sat 10h) — slot 給 spore-publish
...
白天 morning chain（v2.7 避開 9am-5pm 工作時段；v⁹ 加 F）：a (06h) → **S (06h30)** → **F (07h)** → P (08h) → **M (08h30)**，全部在 09:00 哲宇上班前跑完。F (feedback-triage) 把讀者回報開成 from-feedback issue,接著 M (maintainer-am) 同 cycle 收割 → 當天閉環。Evening chain（v2.7 新增）：**X (17h30)** → r (18h) → m (22h) → A (23h)，spore-publish 從 SPORE-INBOX 挑 1 條 17:30 ship → 18:00 rewrite-daily 全 cycle ship → 22:00 maintainer-pm → 23:00 refresh-pm。refresh-am 吃前一日夜間累積，audience flywheel harvest (S) 走 Chrome MCP 完整 cycle (metrics + reply content + factual challenge classify + EVOLVE trigger + reply draft + observer alert)，spore-pick (P) propose 今日 3 candidates 補 SPORE-INBOX，maintainer-am (M) 收割 overnight contributor PR backlog。
...
cron: '30 6 * * *' # 每天早上 06:30（v2.7 audience flywheel cycle 需 60-90 min → 提前 30 min。morning chain：refresh-am 06:00 → spore-harvest 06:30 → spore-pick 08:00 → maintainer-am 08:30）
model: opus
...
cron: '0 8 * * *' # 每天早上 08:00（v2.7 morning chain 中段，refresh-am 06h → spore-harvest 06h30 → spore-pick 08h → maintainer-am 08h30，整條 chain 在哲宇 09:00 工作時間前跑完 — 哲宇 2026-05-23 directive「routine 盡量放早上 8 點前不會撞工作時間」+ 2026-05-27 v2.7「9am-5pm 都不要卡」）
...
cron: '30 17 * * *' # 每天下午 17:30（v2.7 evening chain output 棒，避開哲宇 9am-5pm 工作時段 + 接在哲宇下班直後社群 prime time 起點，跟 rewrite-daily 18:00 形成 evening publish chain：17:30 publish from inbox → 18:00 rewrite full cycle ship）
...
prompt: |
  ...
  Stage 2 QUALITY GATE：4 條 hard gate 全過才 ship（任一不過 → 找下一條）：
  - prose-health ≥ 8.0 / word-count ≥ 4500 CJK / footnote-density ≥ B
  - media-richness pass (iframe ≥ 1 AND image ≥ 2) / lastVerified ≤ 90 天
...
crons:
  - '30 8 * * *' # am 08:30 白天（v2.7 from 09:00：避開哲宇上班時段）
...
cron: '30 0 * * *' # 每天 00:30（v2.8 shift：原 05:00 → 00:30，避開 06:00 morning chain；4hr 49min worst case 仍剩 41 min buffer）
model: opus
skill: /twmd-babel
