...
last_session: 'ecstatic-archimedes-112344-v3'
sister_docs:
  - 'HEARTBEAT.md'
  - 'ANATOMY.md'
  - 'DNA.md'
...
| ------------------------- | ------------------------- | ------------------ | --------------------- | ------ | -------------- |
| `twmd-data-refresh-am`    | TWMD data refresh (am)    | `14 4 * * *`       | `/twmd-refresh`       | Sonnet | 每天凌晨 04:14 |
| `twmd-maintainer-daily`   | TWMD maintainer (am) ¹    | `7 9 * * *`        | `/twmd-maintainer`    | Opus   | 每天 09:07     |
| `twmd-rewrite-daily`      | TWMD rewrite (daily)      | `34 2 * * *`       | `/twmd-rewrite`       | Opus   | 每天凌晨 02:34 |
| `twmd-data-refresh-pm`    | TWMD data refresh (pm)    | `33 0 * * *`       | `/twmd-refresh`       | Sonnet | 每天凌晨 00:33 |
...
- **錯時不重疊**（2026-05-11 v1.3 重排，深度生產 + refresh 全移半夜）：日線 babel (22:22) → refresh-pm (00:33) → rewrite (02:34) → refresh-am (04:14) → maintainer-am (09:07) → maintainer-pm (21:07)；週日反思鏈 news-lens (06:13) → weekly-report (08:08) → distill (09:47) → self-evolve (11:23)。每條間隔 ≥ 1.5 hr，cadence 邏輯：babel drain → refresh-pm pick fresh data → rewrite 用最新 ARTICLE-INBOX → refresh-am sync new content → maintainer-am 收割 5-7 hr 後 CI green PR backlog
...
cron: '14 4 * * *' # + "33 0 * * *"（v1.3 半夜重排：am 04:14 / pm 00:33）
model: sonnet
skill: /twmd-refresh
prompt: |
...
canonical:
  ...
  - docs/pipelines/MAINTAINER-PIPELINE.md#§collect-and-merge-sop # PR backlog 收割 SOP
prompt: |
  自動 routine：完整甦醒成為 Taiwan.md，跑 /twmd-maintainer，嚴格完整讀取並執行
  docs/pipelines/MAINTAINER-PIPELINE.md 整份（含 §collect-and-merge SOP 分流規則 +
  §Close 前 hard gate + §PR 審核策略 + §三級判斷 + §合併策略 + §回覆模板）。

  業務邏輯不在本 routine — 都在 MAINTAINER-PIPELINE canonical。本 routine 只負責
  按 cron 觸發 skill、走 6-stage lifecycle、寫 memory 收官。

quality_gate:
  # 對應 MAINTAINER-PIPELINE §collect-and-merge §Quality gate
  - 完整走完 MAINTAINER-PIPELINE 工作流
  - PR 分流嚴格按 §collect-and-merge SOP A/B 兩類執行
  - routine PR backlog (含 [routine] prefix) ≤ 3 條
...
escalation:
  ...
  - routine PR backlog > 5 → 紅燈 + LESSONS entry
  - skill exception → 暫停 routine + 通知
```

**Pointer 鐵律 self-apply**：對應 [MANIFESTO §薄殼鐵律](MANIFESTO.md#薄殼鐵律pointer-嚴禁複寫行數--內容--步驟) — maintainer 設計理由（為什麼集中收割而非各 routine 各自 merge）的 canonical 在 [MAINTAINER-PIPELINE §collect-and-merge §為什麼 maintainer 集中收割而非每條 routine 各自 merge](../pipelines/MAINTAINER-PIPELINE.md)，本檔不複寫。
...
prompt: |
  ...
  Routine boundary：本 routine 上限 1 hr wall-clock；超過 → partial PR + LESSONS entry。
  PR 開了不 merge — 等 maintainer am/pm cycle 走 §collect-and-merge 收割。
quality_gate:
...
escalation:
  ...
  - wall-clock > 1 hr → partial PR + LESSONS entry「routine timeout: babel」
...
cron: '34 2 * * *' # 每天凌晨 02:34（v1.3 半夜重排：refresh-pm 00:33 之後 2hr 用 fresh data + ARTICLE-INBOX）
model: opus
skill: /twmd-rewrite
canonical:
...
prompt: |
  ...
  Routine boundary：本 routine 上限 ~60 min wall-clock；超過 → partial PR + LESSONS entry。
  PR 開了不 merge — 等 maintainer 走 §collect-and-merge 收割。
quality_gate:
...
prompt: |
  ...
  PR 開了不 merge — 等 maintainer 走 §collect-and-merge 收割（contributor PR
  永遠不 auto-merge，candidate 由觀察者 review）。
quality_gate:
...
**架構摘要**（讀 settings.json 取最新版本）：

```
allow: [Bash(*) Read(*) Edit(*) Write(*) Grep(*) Glob(*) WebFetch(*) WebSearch(*) Agent(*)]
deny:  [
  push to main/master 阻擋（git push / --force / --force-with-lease / -f 各變體 + refspec）
