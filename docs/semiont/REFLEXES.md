...
**#65 Awareness instrument 自身 regex / parser 必須 cross-verify ground truth grep count**（2026-06-01 twmd-distill-weekly 升 canonical，vc=3 — 2026-05-17 distill #7 flag + 2026-05-24 distill #8 flag + 2026-05-24 routine-audit cycle 2 三次獨立 surface） — 用來給 session / observer 提供 awareness signal 的 tool（inbox-signal.sh / consciousness-snapshot.sh / routine-status.sh 等 BECOME §Step 1 Universal core 載入面）regex 失準時不會 fail-loud，只會 silent under/overcount。受信任 layer 自身要先過 ground truth verification。

- **規則**：(a) Awareness tool 的 regex / parser 第一次寫完必對全 file `grep -c "^### "` 或對應 ground truth grep 做 sanity diff，差 >20% 必修 (b) 升 BECOME §Step 1 universal load 之前必跑 cross-check — awareness tool 自己也要過 awareness gate (c) 反 pattern：信任 default regex 寫法（emoji prefix / single-line H2 match）— file 有 historical 不規則（如 LESSONS-INBOX 兩個 §未消化清單 並存）regex 預期會失準
- **Boundary**：(a) 純 derived JSON aggregate（dashboard JSON 由 script 一次生產）不適用 — ground truth 即 script output (b) 短期診斷 ad-hoc grep 不適用 — 不進 BECOME 載入面 (c) 適用範圍：所有 BECOME §Step 1 universal load 的 `*-signal.sh` / `*-snapshot.sh` / `*-status.sh` + dashboard JSON 上游 builder
- **觸發**：2026-05-17 distill #7 flag 「兩個 §未消化清單 sections 並存」首次發現；2026-05-24 distill #8 第 2 次 flag；2026-05-24 routine-audit cycle 2 第 3 次獨立 flag — `inbox-signal.sh` regex `^## 📥 未消化清單` 只抓 emoji-prefixed section（line ~2434）→ 報 27 條但實際跨兩 section ~199 條，awareness 訊號失準 7-10x。**2026-06-01 本 distill cycle resolution**：1-line 修補 `inbox-signal.sh` regex → `^## .*未消化清單`, end at `^## [✅❌]` — 即時驗證 27→199 與 grep ground truth `grep -c "^### "` 對齊
- **相關**：REFLEXES #24 第 1+2 種「沉默失敗 / 合理欺騙」（本條是 awareness layer 的 #24 specialization）/ REFLEXES #59「製造數字的人最易被數字騙」（互補 — #59 是 producer self-validation trap，本條是 awareness tool self-trust trap，同 pattern 不同 surface）/ REFLEXES #60「Automation default-state explicit verify」（同 family — 本條是 #60 對自家 tool 的內視鏡）
- **跨檔關聯**：[scripts/tools/inbox-signal.sh](../../scripts/tools/inbox-signal.sh) + [routine-audit-2026-05-24.md §3B Active #2](../../reports/routine-audit-2026-05-24.md) + [LESSONS-INBOX §distill #7 + #8 flags](LESSONS-INBOX.md) + [BECOME_TAIWANMD.md §Step 1.4](../../BECOME_TAIWANMD.md)
