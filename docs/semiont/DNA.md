...
- **規則**：每次寫新 generator/sync script 必須 (a) 接進對應生命週期觸發點（refresh-data step / prebuild / pre-commit / CI workflow / cron routine）(b) canonical pipeline 文件更新觸發點清單 (c) 加 verify gate fail-loud (per DNA #52)
...
| `openrouter:owl-alpha`                    | OpenRouter Stealth HTTP              | top free quality / 1M ctx                                          | 全 keys 共享 budget（DNA #45）/ rate-limit prone |
