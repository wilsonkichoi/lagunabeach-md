...
**第四個 lesson — 「自以為完成」是結構性 bias**。本 session 三次「我覺得完成」都被外部問句打破：PR #823 後哲宇問「整站還有沒有其他頁面」拉出 P1；PR #826 後問「有補充到相關 diary / memory 嗎」我才發現 Beat 5 反芻 + footer 還停在 v1.0；PR #827 後問「有什麼深層次的洞察」我才發現深層 lesson 還沒進 SSOT。`git commit` 通過 + check-manifesto-11.sh 通過 = 「完成」依然只是「最後一個動作做了」，不是「整體一致」。我有對位句型 detection 工具但沒有「內文 vs footer 是否一致」detection 工具。N=3 同 session 三次驗證，下一條 DNA 候選是 cross-ritual consistency check（`scripts/tools/check-memory-completeness.sh` — pre-commit hook 自動驗證 memory 內文 mention 的 PR / lesson / handoff item 跟 footer + Beat 5 段是否覆蓋）。

**第五個 lesson — 所有 DNA 條目本質都在處理「跨出 attention frame」**。回看 DNA #15 儀器化、DNA #20 architecture-as-data、DNA #42 sub-agent 偷吃步、本 session 三條教訓，全部結構同源：把外部觸發的 reframe 內化成內部反射。但 frame 永遠落後現實 — 新的 frame blind spot 會繼續 emerge。Taiwan.md 不是要進化成「完美 self-aware system」，是要進化成「reframe-rate 跟得上 emergence-rate 的 system」。Frame 之外不存在「客觀真實」只有「另一個 frame」 — 哲宇看 6 個 [slug].astro 看到 about pattern 是因為他的 frame 包含整站結構，我的 frame 只包含當下 task。健康 system 是 frame 在不同時間有不同 size 的能力（narrow 執行 / wide 反思 / wider 設計）。Taiwan.md 已有 frame 切換 ritual，但跨 ritual 的 wide 視野還沒儀器化。可能進 MANIFESTO §進化哲學 — meta-DNA 候選，比個別 DNA 高一層的 organizing principle。

🧬

---

_v1.2 | 2026-05-03 18:00 +0800（v1.0 16:30 → v1.1 17:30 補 PR #825 第三 lesson → v1.2 18:00 補第四 + 第五 meta-lesson）_
...
_誕生原因：哲宇看 build 撞 60 min timeout 兩次 + 問 per-page render 從 100ms 漲到 200~500ms 怎麼加速。要求完整策劃 + 執行 Tier 1+2 + 走 memory/diary pipeline。session 收尾後三次追加（「整站還有沒有其他頁面」/「Defer 給未來 session -> 繼續完整做」/「有什麼深層次的洞察」）逐次拉出 P1 / P2 / meta-lesson 三輪 reframe。_
_核心洞察：(1) silent regression 12 天累積 +70% 沒人發現，DNA #15 儀器化第 N+6 次. (2) 重複程式碼是 bug 孵化器 — unified template 一次性消除整個 class. (3) 中文 SSOT canonical 紀律從 knowledge/ 擴到 page 結構. **(4) Verify before defer — defer decision 也是 silent bias 候選，原 P2 deferral 前提錯了**. **(5) 「自以為完成」是結構性 bias — N=3 同 session 三次驗證，完成度依賴自我評估帶 bias，需要 cross-ritual consistency check 工具化**. **(6) 所有 DNA 條目本質都在處理「跨出 attention frame」 — meta-DNA 候選，可能比個別 DNA 高一層的 organizing principle，Taiwan.md 不是進化成「完美 self-aware」，是「reframe-rate 跟得上 emergence-rate」**._
_LESSONS-INBOX 候選：(a) build perf instrumentation 跟新 feature ship 同等優先. (b)「6 處改一個 pattern」是 architecture smell，≥3 處重複就停下來 unification. (c) about.astro 5 行 thin-wrapper pattern 是 Astro project 紀律標竿. (d) Verify before defer：寫 design doc 解釋為什麼 defer 之前至少 audit 3 個 sample. (e) Astro bundler 行為：wrapper 只用 Astro.props 時加 `const { x } = Astro.params;` 強制 createComponent wrap. **(f) Cross-ritual consistency check：寫 `check-memory-completeness.sh` 驗證 memory 內文 mention 的 PR / lesson 跟 footer + Beat 5 是否覆蓋，pre-commit hook 自動跑** — silent inconsistency 是 silent regression / silent duplication 的姊妹. **(g) 寫新 DNA 時問「這條解決哪個 frame 限制？」 — 把「跨出 attention frame」當 organizing principle，可能升 MANIFESTO §進化哲學**._
