...
## Supplement (post-merge ~17:55 +0800)

**Production verify (PR #828 deploy 7b120c1d 後)**：6 篇抽樣 → 5/6 ✓（zh-TW 卓榮泰 / en Ang Lee / ja A-Mei Noto Serif JP / ko 린유자 Noto Serif KR / 民歌運動 全 200 + 視覺保真），1/6 fail（diary musing-chaplygin 404）。Diary 404 root cause: v3 v4 共有 pre-existing default 不含 diary。

**v4.1 fix（PR #829 8ac47e3c）**：generator default 改 articles + diary 一起跑，加 `--no-diary` opt-out / 保留 `--include-diary` alias。下次 deploy 自動全量 regen ~93 diary OG。

**Q1 diary 排序 fix（PR #832 966fc267）**：哲宇 callout「musing-chaplygin 為什麼不排第一」→ 揭露 named sessions 全 fall through `greekOrder = 0` → FS alphabetical bug。修補：sort 加 `mtimeMs` 第三 tie-breaker，CI git-restore-mtime 還原 commit time。

**Q2 Owl 巴別塔 diary 翻譯 POC**：3 篇 sample → 2 篇 ✓（musing-chaplygin / 安溥 viral，37-46s/each、ratio 2.37-2.66、無 refusal）+ 1 篇 rate limit 429（DNA #45 第 N+1 次驗證 — 3 sequential req < 2min 撞 cool-down）。翻譯品質意外高（Owl 處理 narrative voice + MANIFESTO 哲學 nuance 都到位）。完整 scope assessment：475 翻譯 + 6-10 hr 前端工程 = 16-30 hr，超 single session 邊界 → 落 [reports/owl-diary-translation-poc-2026-05-03.md](../../reports/owl-diary-translation-poc-2026-05-03.md) + 三選項給哲宇 next session 決定。
