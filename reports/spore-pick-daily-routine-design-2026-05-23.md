...
execution_mode: 'local cron' # 哲宇 post-ship 校正：跟其他 13 routine 一致跑 local
schedule: '0 8 * * * Asia/Taipei (每天 08:00)'
related:
...
**結論（2026-05-23 ship complete）**：新增獨立 routine `twmd-spore-pick-daily` 每天 **08:00**（Asia/Taipei）Sonnet 跑 `/twmd-spore-pick` skill，propose 3 candidates append [SPORE-INBOX.md](../docs/factory/SPORE-INBOX.md) §Pending。news sense 整合走 weekly news-lens 升級（直接 append SPORE-INBOX 5-7 news-driven candidates P1）喂給 daily routine。容量管理由既有 weekly distill 接管（pending ≥ 30 觸發觀察者 attention，≥ 50 auto-drop routine-source 最舊 5 條未 promote entries）。

**Execution mode**: local cron（哲宇 post-ship 校正 — 跟其他 13 routine 一致 local 跑，遠端 RemoteTrigger 初版已棄）
**Schedule**: 每天 08:00 Asia/Taipei
...
- [x] 建 cron task `twmd-spore-pick-daily` — initial 透過 RemoteTrigger API ship 後，哲宇 post-ship 校正改 local cron（跟其他 13 routine 一致 execution mode），首次跑 2026-05-24 08:00 Asia/Taipei
...
_Execution mode: local cron（initial RemoteTrigger 已棄，哲宇 post-ship 改 local 跟其他 13 routine 一致）/ Schedule: 每天 08:00 Asia/Taipei_
