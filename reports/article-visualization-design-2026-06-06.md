# 文章視覺化系統設計報告 — 偷 The Pudding 的靈魂，不偷它的引擎
...
**核心主張（thesis）**：Taiwan.md 該偷 The Pudding 的**編輯哲學**（問題先於資料／結論要明確／讀者是偵探／annotation 是主角），但**拒絕它的技術棧**（D3 / Svelte / Canvas / OAuth 互動）。因為 Taiwan.md 的三個約束——靜態 `.md` SSOT、六語、以及最關鍵的 **AI-SEO／主權使命**——讓「比較笨」的技術路反而**比 Pudding 的互動路更強**：
...
## 2. The Pudding 深度 study — 偷什麼、不偷什麼
...
| **B. Interactive Explorer**    | 讀者篩選/輸入/玩                       | [Judge My Spotify](https://pudding.cool/2021/10/judge-my-music/) / [Hip Hop Vocabulary](https://pudding.cool/2017/02/vocabulary/) beeswarm             | 🔴 **不偷**（OAuth/大型篩選需後端+重 JS，破壞 AI 可讀性）            |
| **C. Annotated Static Charts** | 靜態圖 + 直接標註，詞少資料密          | [Birthday Effect](https://pudding.cool/2025/04/birthday-effect/)（annotation 是主角）                                                                  | 🟢 **核心要偷**。SEO/AI/多語最友好，是 Taiwan.md 主力模板            |
