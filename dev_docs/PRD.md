...
1. **lagunabeach.md** — a design-quality, AI-native knowledge base for Laguna Beach, CA:
   ...
   fork (archived as `lagunabeach-md-v1`) at the same domain (ADR 001; ROADMAP Phase 3).
...
- The fork's maintenance cost was structural, not incidental: the last upstream merge
  ...
  day job; de-Taiwan maintenance was unbounded (ADR 001).
- A previous greenfield attempt (v0) failed because design was described in prompts; the
  hybrid rebuild copies the actual design system as files, removing that failure mode (ADR 001).
- Framework-first with zero instances would repeat Taiwan.md's coupling mistake in mirror
  ...
  sequenced ahead of the features Wilson personally wants most (ADR 002).
...
Phase-gated proof points rather than a single metric (each is a ROADMAP acceptance criterion):
...
- **Semiont is optional**: the site must build with the `semiont/` directory deleted; every
  organ beyond the minimal core is opt-in (ADR 003).
