#!/bin/bash
# LagunaBeach.md 一鍵releasescript
...
echo "🇹🇼 LagunaBeach.md releaseflow"
echo "================================================="
...
if [ -f scripts/sync.sh ]; then
 ...
 echo "⚠️ Not foundsyncscript，manualsync..."
...
if [ -f scripts/fix-all-frontmatter.py ]; then
 python3 scripts/fix-all-frontmatter.py 2>/dev/null || echo " ⚠️ frontmatter FixSkipped"
fi

# Step 3: Count articles
echo ""
echo "📊 Step 3/5: statisticsArticles數"
...
echo " 🇹🇼 ChineseArticles: $ZH_COUNT"
echo " 🇺🇸 EnglishArticles: $EN_COUNT"
