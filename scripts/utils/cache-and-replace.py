...
print(f"🖼️  Found {len(all_urls)} Wikimedia URLs")
...
for url in sorted(all_urls):
    ...
    try:
        req = urllib.request.Request(url, headers={
            'User-Agent': 'LagunaBeachMD/1.0 (https://lagunabeach.md; educational project)'
        })
...
print(f"\n📊 Download: {downloaded} new + {cached} cached = {len(url_to_local)} total")
...
print(f"\n🔄 Replaced: {urls_replaced} URLs in {files_changed} files")

# Step 4: Summary
total_files = len([f for f in os.listdir(IMG_DIR) if not f.endswith('.txt')])
print(f"📂 Cache directory: {IMG_DIR} ({total_files} files)")
print("🎉 Done! Images will never go dead")
