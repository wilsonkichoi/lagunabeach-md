#!/usr/bin/env python3
"""
instrumentation-audit.py — GA4 event-param ↔ custom-dimension driftDetect器（Immunity Layer 1）

防的 bug class：「instrumentation code 送什麼 param」跟「GA4 註冊了哪些 custom dim」是兩
分離的真相，由兩differentflowMaintenance（改 code vs 點 GA4 Admin / 跑 register script），drift是靜默的。
2026-05-29 D+2 watch 一次Fetch到三 instance：
 - pct: dim 註冊了但 code 送 depth_pct（名characters對不上 → attribution 全 not-set）
 - link_url: code 送了但From沒註冊（→ query 端瞎掉）
 - failed_path: page_404 送了但From沒註冊（→ fetch-ga4 query 一直 try/except 吞掉）

把 code 當 SSOT（跟「knowledge/ 才是 SSOT」sameprinciple往 instrumentation 層延伸），三方alignment：
 (1) code actual fire 的 param — Parse tracker original碼
 (2) register script declare的 SSOT — register-ga4-custom-dimensions.py 的 *_DIMENSIONS list
...
# python 跑 --static（純 stdlib）。Must在 google import 前，且只在directlyExecute時。
...
# tracker original檔 — any _fire() / gtag('event', ...) 埋點的file都要列在這
TRACKER_FILES = [
...
# 故意不註冊成 custom dimension 的 param（例：純 debug、或 GA inside建已covers）。
# 空 = all fire 的 param 都該有 dim。加進來的每一都要附理由。
IGNORE_PARAMS = set()


# ── 1. Parse code：抽出Each event actual fire 的 param keys ────────────────────
def _balanced_object(text, start):
 """From text[start] 的 '{' start，return到 matching '}' 的 substring（含兩端 brace）。"""
    depth = 0
...
def _top_level_keys(obj_str):
 """抽出 object literal depth-1 的 key 名（避開 nested object + ternary 的 colon）。"""
    keys = []
    depth = 0
 expect_key = False # 剛遇到 depth-1 的 { 或 , → 下一 token 可能是 key
    i = 0
    while i < len(obj_str):
        c = obj_str[i]
        if c in "{[(":
...
def parse_fired_events(files):
 """return {event_name: set(params)}，掃all _fire(...) 與 gtag('event', ...) call。"""
    events = {}
 # 兩種call起點：_fire( / gtag('event', / gtag("event",
 ...
 # 注進Each event）— call-site object literal 看不到，但Each event 都帶。
...
# ── 2. Load register script declare的 SSOT ────────────────────────────────────
def load_ssot():
    spec = importlib.util.spec_from_file_location("register_dims", REGISTER_SCRIPT)
    mod = importlib.util.module_from_spec(spec)
 spec.loader.exec_module(mod) # Safety：google import 在 main() inside、re-exec guard 看 __main__
...
def main():
 ...
 ap.add_argument("--json", action="store_true", help="Output JSON")
    ...
    if run_static:
 # code 送了但 SSOT 沒declare（且not in ignore/deprecated）→ query 端會瞎（link_url / failed_path class）
 ...
 errors.append(f"[code↔SSOT] param `{p}` 被 fire（{', '.join(evs)}）但 register script SSOT 沒declare → 加進corresponding *_DIMENSIONS")
 # SSOT declare了但沒any event fire（且非 deprecated）→ 可能死 dim（pct class）
        declared_unfired = declared - fired - deprecated
        for p in sorted(declared_unfired):
 warns.append(f"[code↔SSOT] dim `{p}` 在 SSOT 但沒any event fire → 可能已死，考慮加進 DEPRECATED_DIMENSIONS")

    if run_live:
        try:
            live = load_live_dims()
            info.append(f"GA4 live dims: {len(live)}")
 # SSOT declare了但 GA4 沒註冊 → 跑 register script
 ...
 # GA4 註冊了但 SSOT 沒declare（非 deprecated）→ manual註冊沒 codify（5/27 class）
            live_undeclared = live - declared - deprecated
            for p in sorted(live_undeclared):
 warns.append(f"[SSOT↔GA4] GA4 有 `{p}` 但 SSOT 沒declare → manual註冊未 codify，加進 *_DIMENSIONS")
 ...
 print("🔬 instrumentation-audit — GA4 event-param ↔ custom-dim alignment")
        for line in info:
            print(f"   ℹ️  {line}")
 ...
 print(" ✅ code ↔ SSOT ↔ GA4 三方alignment，無drift")
        elif not errors:
