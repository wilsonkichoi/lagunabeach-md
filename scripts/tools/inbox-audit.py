#!/usr/bin/env python3
"""inbox-audit.py — ARTICLE-INBOX 對地真相健檢 + Distill SafetyExecute器.

把「查現況」儀器化：一指令交叉Compare ARTICLE-INBOX §Pending 的Each entry 對
`knowledge/`（Articles是否Already exists）+ ARTICLE-DONE-LOG（是否已歸檔），category出幽靈、
duplicate、真待辦，Output triage report。--apply-safe 只Remove 100% 明確的幽靈（self-declared
done 或 Already exists且已 logged），帶 line-conservation 保證（非只 entry count）。

誕生：2026-06-19-123909-inbox-distill session — manual distill 95→79 Fetch出 16 幽靈
（Done歸檔鐵律無structure強制 → inbox drift）。Cheyu directive：把這次的handle/分析儀器化。
canonical SOP：docs/semiont/ARTICLE-INBOX.md §Distill SOP
designlesson：REFLEXES #15（反覆浮現要儀器化）+ #38（混維度 silent killer：count 對但Content掉）

Usage：
 inbox-audit.py # triage report（human markdown）
 inbox-audit.py --json # machine-readable（routine / signal 消費）
 inbox-audit.py --apply-safe # 只Remove明確幽靈（done / exists+logged），line-conservation 保證
 inbox-audit.py --apply-safe --dry-run # 預覽要Remove什麼，不寫檔
category：
 🔴 DECLARED-DONE status=done/dropped/已Done — Done卻沒搬走（鐵律違反），--apply-safe 唯一會動的類
 🟠 STALE-NEW Type=NEW 但ArticlesAlready exists — NEW 已被滿足，待人工 review
 🟣 PARTIAL-SHIP prose-shipped-pending-media 類 — 正文 ship、媒體/babel 待補（合法 pending）
 🟡 EVOLVE-PENDING Type=EVOLVE + Articlesexists + pending — 合法 re-EVOLVE（Articlesexists＋可能已在 DONE-LOG 是前提，非幽靈）
 ✅ GENUINE-PENDING ArticlesDoes not exist + pending — 真待辦
 ⚪ SERIES 系列 umbrella / Tier / pick list — 不 auto-resolve，人工確認整批
 🔁 DUP ≥2 entry 解到sameArticles
--apply-safe 只動 🔴 DECLARED-DONE（status 自宣Done＝最Safety訊號）。其餘一律留人工（κ 5-PR lesson：curation 不batch自決）。
"""
...
SERIES_MARKERS = ("系列", "batch", "umbrella", "series", "共通說明", "pick list",
                  "Tier", "Round 2", "P0×5", "P0x5", "candidate）", "Peek")
...
def classify(entry, donelog_text):
 ...
 and it may already be in DONE-LOG from a prior ship (造山者/沈伯洋/蔡English case)."""
 ...
 note = "（已在 DONE-LOG，確認是 re-EVOLVE not幽靈）" if logged else ""

    if declared_done:
 return "DECLARED-DONE", "🔴", "status 自宣 done/dropped — Done卻沒搬走（鐵律違反），可SafetyRemove"
    if is_series:
 ...
 return "EVOLVE-PENDING", "🟡", f"Articlesexists({art}) + EVOLVE pending — 合法 re-EVOLVE{note}"
 if art: # NEW (or unknown type) but article exists → NEW 已被滿足
 return "STALE-NEW", "🟠", f"NEW 但ArticlesAlready exists({art}) — NEW 已滿足，待人工 review 是否Remove{note}"
 return "GENUINE-PENDING", "✅", "ArticlesDoes not exist + pending — 真待辦"
...
def main():
 ...
 print(f"=== --apply-safe: Remove {len(removed)} 明確幽靈（DECLARED-DONE，status 自宣Done）===")
        for h in removed:
            print("  - " + h)
        print(f"line-conservation + section-survival: {'OK ✅' if ok else 'BROKEN ❌'}")
        if not ok:
 print("中止：line-conservation Failed（拒絕 silent Content流失）")
            return 2
        if args.dry_run:
 print("\n(dry-run；拿掉 --dry-run 才寫檔)")
        else:
            INBOX.write_text(new_text, encoding="utf-8")
 print(f"\n*** 已Write {INBOX.relative_to(ROOT)} ***（記得 ship DONE-LOG backfill + git）")
        return 0

    # human triage report
    print(f"# ARTICLE-INBOX audit — {len(entries)} 個 pending entry\n")
 print(f"摘要：" + " / ".join(f"{c}={counts[c]}" for c in order if counts[c]))
 print(f"→ 🔴 可SafetyRemove {len(removable)} （--apply-safe）；🟠 STALE-NEW + ⚪ SERIES 待人工 review\n")
    for c in order:
 ...
 conf = {"fuzzy-stem": " ⚠fuzzy確認scope"}.get(e["resolved_by"], "")
    ...
    if dups:
 print(f"\n## 🔁 DUP（{len(dups)} Articles被多 entry 指到）")
