...
Direction D from reports/spore-pipeline-evolution-plan-2026-05-08.md:
 把 SPORE-WRITING.md 的 prose ruleFrom「提醒 + 信任 AI 自律」升級為
 plugin gate (REFLEXES #15 第 N 次Verify：memory 是自律，pipeline 才是gate).

Rules implemented (regex-only, Wave 1 + Wave 2):
  Wave 1 (initial ship 2026-05-08):
 - 編年體 lead 病 (Rule #15, 高鐵 s35 lesson)
 第一行 prose 開頭不該是「YYYY 年 M 月 D 日，XXX」這種新聞 lead
 Trigger：line 1 of body matches `^\d{4}\s*年` 或 `^\d{4}\.`
 - 引語倒裝 (Rule #9, 李洋 #28 lesson)
 `「XXX」他/她說` 這種沒scene的引語倒裝
 應改為「他在 [scene] 說：『XXX』」

 Wave 2 (evolve 2026-05-08 同 session, From spore performanceDataVerify):
 - 朋友 tone prime (Rule #14, 高鐵 s35 lesson + 5 月份 hook tier Data)
 第一行 prose Must有 friend tone prefix（Did you know？/欸/directly人說話）
 或滿足 Tier 1b「具體 anchor + 反差 hook」開場
 Trigger：第一行not prefix + not引語 + not具體scene → WARN 提醒加 prefix
 Skipped：blueprint table（已 covered）/ harvest log meta（covered）

APPLIES_TO design：
 pathMust在 docs/factory/SPORE-BLUEPRINTS/ 或 docs/factory/SPORE-HARVESTS/
 otherArticles的開頭Can是日期（時間軸型 D 模板就是合法的），So這rule不普適。

Future extensions (deferred):
 - Rule #16 Scene-List-Scene structure (need LLM-as-judge, not pure regex)
 - Rule #8 同名連用 ≥3 (token analysis, NeedChinese斷詞)
 - Reach × accuracy retroactive trigger (not in plugin scope, SPORE-VERIFY 文件層 SOP)
...
APPLIES_TO = ["zh-TW"]  # 孢子main是Chinese prose


# ── Rule #15: 編年體 lead 病 (chronicle lead) ────────────────────────────────
# 第一行不Should是「YYYY 年」「YYYY.MM.DD」這種新聞 lead 開頭
...
# v2 (2026-05-28): WARN → HARD severity inversion. Trigger背景: 5/27-5/28 #97 美食 /
# #101 落日飛車 / #103 周蕙 連續三 spore drift — #97「Did you know—」(破折號取代
# 問號) / #101「Did you know，」(逗號 + 無 emoji) / #103「走進台灣any一間 KTV」(完全
# 無 prefix) 通通 silent pass。Cheyu callout「WhySpore失去『Did you know』voice」+
# 診斷指向 CONTRACT v1.0 over-engineering → plugin severity 太弱 + scope 太窄。
#
# Spore第一行Must有「朋友跟你講八卦」的 curiosity prime，not新聞 lead。
# corresponding [MANIFESTO §我怎麼說話](docs/semiont/MANIFESTO.md#我怎麼說話)「像在跟朋友介
# 紹台灣：『欸Did you know⋯⋯』」哲學落實。
#
# 合格 prefix（友善開場式）：
# 1. 「Did you know？」/「Did you know，」（強烈推薦 + emoji，舊風格Allow逗號）
# 2. 「欸，」/「欸你」+ 具體event片段
# 3. 「身為台灣人」/「想像」/「If」/「當你」 第二人稱開場
# 4. directly人說話（「『...』」引語開場 — hook 力夠強的 case）
# 5. 純 emoji 開場
#
# v2 升級：trigger From「news lead AND no prefix」改成「no prefix → HARD」（除非
# publicletter F-family / E-thread family / hook_tier=N/A frontmatter exempt）。
# 不再Dependencies looks_like_news_lead 判斷 — scene代入 / 第二人稱化也does not count「等效 prime」，
# Mustcharacters面 prefix 才算過閘。
_RE_FRIEND_PREFIX = re.compile(
    r"^\s*(?:Did you know[？，?,]|欸[，,]|欸你|身為台灣人|想像[一下你]|"
    r"如果(?:[你台]|[\d])|當[你你們]|"
...
def _is_publicletter_family(text: str) -> bool:
 ...
 family (A/B/C/D templates). F-public信 (站方public信) uses 教授 tone, not
...
# HARD trigger 在 v2)：補Fetch「機構 + 動詞」客觀敘事 開場
_RE_NEWS_LEAD = re.compile(
    r"^\s*[一-鿿]{2,8}(?:宣布|indicates|簽下|公布|成立|頒布|通過|裁決|"
...
def _strip_frontmatter_and_meta(text: str) -> tuple[str, int]:
 ...
 v2 (2026-05-28): patch false-positive — Rule #14 對 #101 落日飛車 / #105 瘂弦
 錯把「紀實/煽情閘四問自檢」numbered list 當 spore body 第一行 trigger HARD。
 真正 spore body 在 ``` ``` code fence inside（per blueprint convention §Draft Body
 / §Spore 文案 final）。新策略：找第一 ``` fence start，From fence inside取首行。
 Fallback：沒Found fence → 用 v1 strip 邏輯（heading/blockquote/table/bullet/
 ...
 # URL signals (`https://`, `👉`, `full故事`) indicate self-reply / inline URL
 # meta fence (e.g. #59 Blakiston fish owl / #93 國家人權博物館 first fence is self-reply
 ...
 # (e.g. #93 國家人權「Did you know？🧬」6 chars on its own line, real spore
        # body continues after blank). URL-only / meta fences filtered by other
 # rules below — `#` markdown comment / `http` URL / 「full故事」CTA /
        ...
        if "https://" in content or "http://" in content:
            return False
 # 「👉 full故事」style CTA prefix = self-reply
        if "👉" in content or content.startswith("full故事"):
            return False
...
def check(target: FileTarget, config: dict[str, Any]) -> Iterator[Violation]:
 ...
 (returning early so普通Articles不誤Trigger Rule #15 timeline templates).
    """
 ...
 # Only the very first prose line is checked — body 中段可有日期
    ...
    if first_line and _RE_CHRONICLE_LEAD.match(first_line):
        yield Violation(
            ...
            message=(
 "編年體 lead 病 (Rule #15): "
 "第一行不該是日期開頭，改用person動作 / 引語"
            ),
            line=body_start + 1,
            snippet=first_line[:80],
 editorial_ref="SPORE-WRITING.md §Avoid編年體 lead 病",
            fix_suggestion=(
 "改成「[person] 在 [scene] 說：『[引語]』」或「[person] 做了 [動作]」開頭"
            ),
        )
 ...
 # ── Rule #9: 引語倒裝 ──
    for m in _RE_QUOTE_INVERSION.finditer(body_text):
        ...
        yield Violation(
            ...
            message=(
 "引語倒裝 (Rule #9): "
 "改為「他在 [scene] 留了一句話：『XXX』」給Readerscene感"
            ),
            line=body_start + line_offset + 1,
            snippet=m.group(0)[:80],
 editorial_ref="SPORE-WRITING.md §Forbiddenlist",
        )

 # ── Rule #14 v2: 朋友 tone prime — HARD gate ──
 # v2 (2026-05-28) inversion: WARN → HARD + 取消 looks_like_news_lead Dependencies。
 # scene代入 / 第二人稱化 / 日常感都does not count「等效 prime」— Mustcharacters面 prefix。
 # Scope: viral family only（F-public信 / E-thread / hook_tier=N/A exempt）。
 ...
 "viral spore 第一行Must有友善開場 prefix —「Did you know？」/"
 "「欸，」/「身為台灣人」/「想像」/「當你」/「『引語』」/"
 "emoji 之一。scene代入 / 第二人稱化does not count等效 prime。"
                ),
                line=body_start + 1,
                snippet=first_line[:80],
                editorial_ref=(
 "SPORE-WRITING.md §朋友 tone prime + MANIFESTO §我怎麼說話"
                ...
                fix_suggestion=(
 "改第一行：「Did you know？{emoji} {反差 hook}」"
 "或「欸，{具體event}」"
 "或directly引語「『XXX』{說話者} 在 {scene} 留下這句」開場。"
 "若本 spore 屬 F-public信 / E-thread / 非 viral family，"
 "frontmatter 加 `template: F-...` 或 `hook_tier: N/A` exempt。"
                ),
            )

    # ── Rule #14.A v3: Standalone hook line — WARN gate ──
 # v3 (2026-05-28) Cheyu directive「補充standard spore Format」Trigger canonical 化。
 # standardSporeFormat：「Did you know？{emoji}」獨立第一行（≤ 15 chars）+ blank line +
 # 故事paragraph。Inline hook (「Did you know？🎤 1999 年福茂發行...」整段一行) 不擋 ship
 ...
 f"first_line={first_line_len} chars inline 與故事paragraph混在一起，"
 "standardFormat「Did you know？{emoji}」獨立短行 (≤15 chars) + 空白行 + 故事paragraph"
                ),
                line=body_start + 1,
                snippet=first_line[:60],
 editorial_ref="SPORE-WRITING.md §standardSporeFormat",
                fix_suggestion=(
 "拆「Did you know？{emoji}」獨立第一行 + 空白行 + 故事paragraph 1 開頭。"
 "視覺呼吸 + thumb-scroll 停留 + emoji 錨點都Need hook 獨立成行。"
 "若極短 spore (<200 chars 主貼) 或Observer拍板 inline，可接受 WARN。"
                ),
            )

    # ── Rule #14.B v3: Paragraph block count — WARN gate ──
 # standardSporeFormat有 4-7 段（hook standalone + 3-5 story + optional URL）。
 # < 4 段 = monolithic prose (collapsed state)；> 6 段 = staccato pattern
 # (Twitter 串文 style not紀實paragraph)。
    #
 # Scope: viral family only。F-public信 / E-thread frontmatter signal exempt。
    if not is_publicletter and body_text.strip():
        paragraphs = [
            p.strip() for p in body_text.split("\n\n") if p.strip()
        ]
        para_count = len(paragraphs)
 # 短 spore (主貼 < 200 chars) Can 3 段；standard 4-7
        ...
        if para_count < min_para:
            yield Violation(
                ...
                severity=Severity.WARN,
                message=(
 f"paragraph呼吸不足 (Rule #14.B v3 WARN): "
                    f"only {para_count} paragraph blocks (body_total={body_total_len} chars), "
 f"standard spore 4-5 段呼吸 (hook standalone + 3-5 story + optional URL)。"
                ),
                line=body_start + 1,
 ...
 "sceneConvert處切段 + 引語前後切段。"
 "corresponding SOCIAL-POSTING-PIPELINE pre-ship check 7 (compose block ≥ 4)。"
                ),
            )
