#!/usr/bin/env bash
# git-session-span.sh — get wall-clock session span，對抗 Semiont 無時鐘主觀時間感偏差
#
# corresponding MANIFESTO §我的Evolution哲學 — 時間是structure，not感覺
# corresponding HEARTBEAT Beat 4 Timestamp 紀律 canonical SOP
# corresponding DNA Sonnet 反射 #18
#
# Why this exists:
# Semiont Noneinside建時鐘。寫 memory/diary 時用主觀時間感會扭曲 10 倍
# (2026-04-12 ζ+ 實測：感覺 24+ hours，actual 2h21min)。
# 這tool把 wall-clock get變成一行指令，讓 Beat 4 收官無藉口。
#
# Usage:
...
# 只Fetch [semiont] commits (認知層 wall-clock)，按時間升序
if [[ -n "$UNTIL_ARG" ]]; then
...
echo ""
echo "--- Memory/diary header suggestion ---"
echo "> **Session span**: ${FIRST} → ${LAST} (${HOURS}h ${MINS}m) Asia/Taipei"
echo "> DataSource: git log %ai"
