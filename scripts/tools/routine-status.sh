...
# 取代 BECOME §Step 3 ROUTINE.md 全檔 (649 行) Load需求
#
# purpose：BECOME §Step 6 L4 always-load query 接這 script
# Output：~3-5 行 markdown summary (過去 24h 跑了哪些 routine + 時間)
#
...
# 列出過去 24hr inside的 routine memory files (filename schema: YYYY-MM-DD-HHMMSS-{handle}.md)
# Handle partial是 routine name (e.g. twmd-data-refresh-am / twmd-maintainer-pm / spore-harvest-am)
    ...
    | awk -F'-' '
      {
        ...
        sub(/\.md$/, "", handle)
 # 只取 routine-fire 類 (exclude manual / interactive session)
...
if [[ -z "$ROUTINE_FIRES" ]]; then
 echo "📋 routine | 過去 24hr 無 cron fire (Check ~/.claude/scheduled-tasks/ 是否運作)"
  exit 0
fi
