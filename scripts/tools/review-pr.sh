#!/usr/bin/env bash
# review-pr.sh v1.1 — LagunaBeach.md PR automatic審核（五層Immune system）
...
layer0() {
  ...
  for safe in "${SAFE_NON_MD[@]}"; do
    [[ "$f" == "$safe" ]] && echo "✅ 內容配套file（${f##*/}）" && return 0
  done
 [[ ! "$f" =~ \.md$ ]] && echo "🔴 非 .md 檔" && return 1
  if [[ "$f" =~ ^knowledge/ ]] || [[ "$f" =~ ^src/content/ ]]; then
 echo "✅ 純Contentfile"; return 0
  fi
  [[ "$f" =~ \.github/|^scripts/|^src/pages/|^src/components/|^tools/|^package|^astro\.config ]] \
 && echo "🔴 SystemfileForbiddenModify" && return 1
 echo "🔴 非Contentpath"; return 1
}

# ════════════════════════════════════════
# Layer 1 — FormatVerify
# ════════════════════════════════════════
layer1() {
  local f="$1"
 [[ ! -f "$f" ]] && echo "🔴 fileDoes not exist" && return 1
  local err=() wrn=()

  # conflict markers
 grep -qE '^(<{7}|={7}|>{7})' "$f" 2>/dev/null && err+=("Git conflictMark")
 ...
 echo "✅ frontmatter full"; return 0
  fi
}

# ════════════════════════════════════════
# Layer 2 — Qualitygate（inline quality-scan Detect）
# ════════════════════════════════════════
layer2() {
  local f="$1"
 [[ ! -f "$f" ]] && echo "🔴 fileDoes not exist" && return 1
 ...
 # quality: 空洞修飾詞
 local hw; hw=$(echo "$body" | grep -cE '不可或缺|深遠的impact|重要的角色|不可忽視|舉足輕重|密不可分|息息related|獨樹一幟|博大精深' 2>/dev/null || echo "0"); hw=${hw//[^0-9]/}; hw=${hw:-0}
  (( hw >= 3 )) && ((hs++))
 ...
 # quality: 塑膠句式
 local pl; pl=$(echo "$body" | grep -cE 'not.*而是|不僅是.*更是|From.*到.*From.*到|展現了.*也體現了' 2>/dev/null || echo "0"); pl=${pl//[^0-9]/}; pl=${pl:-0}
  (( pl >= 1 )) && ((hs++))
...
layer3() {
 ...
 # 腳註Check（bridge-building鋪路: 讓新Articles天生帶腳註）
...
# ════════════════════════════════════════
# Layer 4 — structureVerify（Stage 4 FORMAT CHECK）
# ════════════════════════════════════════
layer4() {
  local f="$1"
  [[ ! -f "$f" ]] && echo "—" && return 0
 ...
 # 30 second overview
 echo "$body" | grep -qE '>\s*\*\*30\s*second overview|^## 30 second overview' 2>/dev/null || wrn+=("缺30second overview")

 # ## referenceData heading (only if has footnotes)
  local fns; fns=$(grep -cE '^\[\^[0-9a-zA-Z_-]+\]:' "$f" 2>/dev/null || echo 0); fns=${fns//[^0-9]/}
  if (( fns > 0 )); then
 echo "$body" | grep -qE '^## referenceData' 2>/dev/null || wrn+=("有腳註但缺##referenceData")
  fi

 # Further Reading wikilink 殘留
...
review_with_display() {
 ...
 local r0; r0=$(layer0 "$d"); REPORT+=" L0 Safety：${r0}\n"
  ...
  if [[ ! "$d" =~ \.md$ ]]; then
 REPORT+=" L1-L4: ⏭️ Skipped（非 .md 配套檔）\n\n"
 ...
 local r3; r3=$(layer3 "$f"); REPORT+=" L3 Curation：${r3}\n"
  [[ "$r3" =~ ^✅ ]] && L3=$((L3+1))
...
main() {
 ...
 (( ${#files[@]} == 0 )) && echo "❌ Cannotget PR #${pr_num} file" && exit 1
  else
    files=("$@")
  fi
