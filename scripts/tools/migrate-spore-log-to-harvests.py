...
Phase 6 of reports/spore-ssot-pipeline-cleanup-2026-05-08.md (demolish dual-write).

## Why

SPORE-LOG.md metrics table was the narrative SSOT — humans/agents wrote D+N harvest
metrics into rich-text cells. This is SSOT-fragile (parser regressions, format drift).
Phase 6 demolishes it. Before removal, this script preserves all historical D+N data
points by converting them to canonical SPORE-HARVESTS body table format.

## Algorithm

1. Index existing SPORE-HARVESTS body rows by (n, d_n_int) — these are already canonical.
2. Walk SPORE-LOG metrics tracking rows:
...
def parse_spore_log_metrics_rows():
    """Parse metrics tracking table -> list of {n, slug, platform, narrative, struct_views_7d, ...}.

    Returns rich dict including struct cols (7d reach / 7d engagement / 30d reach / 30d engagement)
...
def render_migration_batch(parsed_rows):
    ...
    fm_lines = [
        ...
        f"batch_reason: 'one-time migration from SPORE-LOG metrics narrative SSOT to canonical batch log SSOT (Phase 6 demolition)'",
        ...
        "",
        f"> Phase 6 SSOT cleanup — converts SPORE-LOG.md metrics narrative cells to canonical body table.",
    ...
    fm_lines.extend([
        ...
        "_Source: SPORE-LOG.md metrics narrative cells, parsed and structured for SSOT canonicalization._",
        "_After this migration, SPORE-LOG metrics table is deprecated — see Phase 6 of reports/spore-ssot-pipeline-cleanup-2026-05-08.md._",
    ])
    return "\n".join(fm_lines) + "\n"
...
def main():
    ...
    print(f"  SPORE-LOG metrics tracking rows: {len(sl_rows)}")
