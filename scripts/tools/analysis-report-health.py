#!/usr/bin/env python3
"""analysis-report-health.py — 分析report誠實 gate linter（ANALYSIS-PIPELINE Stage 6）

把「防分析幻覺」的判斷From『我這次記得做』變成『不做就紅燈』。鏡像 research-report-health.py
的架構（stdlib-only / Exit codes / --json / 可接 CI），但檢的是**Data分析report**的誠實脊椎，
not Stage 1 研究report的Search軌跡。

corresponding design report §三 分析幻覺FailedModedirectory H1-H9：
 window-defined → 沒框窗口/邊界 = H4/H6 混淆波/新鮮感失守
 confounder-section → 沒列 confounder = H2/H3/H8 沒exclude
 raw-archived → 沒歸檔 raw = 不可重現
 falsification → 沒證偽段 = H7 selfVerify偏誤
...
Usage:
 ...
 analysis-report-health.py {file} --tier=deep # Default，全 5 hard
...
Source: 2026-06-05 ANALYSIS-PIPELINE bridge-building（design §四 + §七.1 structure性 gate）。
"""
import argparse
import json
import re
...
def load(path):
    p = Path(path)
    if not p.exists():
 print(f"❌ fileDoes not exist: {path}", file=sys.stderr)
        sys.exit(2)
...
def check_window(t):
 ...
 return ok, "需define分析窗口/邊界（兩以上日期 + BEFORE/AFTER 或 attribution 分界）"


def check_confounder(t):
    ok = has(t, r"confounder", r"混淆", r"但書", r"caveat", r"\bH[1-9]\b",
             r"exclude", r"lag", r"attribution.lag", r"流量混入", r"爬蟲", r"crawler")
 return ok, "需有 confounder / 但書段（列出並handle混淆源，at least attribution lag / 流量混入 / 爬蟲其一）"


def check_raw(t):
    ok = has(t, r"reports/research", r"raw.*json", r"\.json", r"original數據", r"附錄.*數據", r"raw dump")
 return ok, "需指向歸檔的 raw 查詢Data（reports/research/*.json）以保可重現"
...
def check_falsify(t):
 ...
 # OR anti-over-attribution / anti-bias self-check (attribution reports). Both守 H7.
    ok = has(t, r"反例", r"反解釋", r"證偽", r"falsif", r"self驗證", r"最強反",
             r"無法.{0,6}exclude", r"也可能是", r"counter", r"anti-bias", r"自檢",
             r"不要把.{0,8}(全給|講大|過頭)", r"別把.{0,12}(講大|講過頭|全給)",
             r"誠實的故事", r"把數據講準", r"不亂歸因", r"過度歸因", r"Q13")
 return ok, "需有證偽/反解釋段（最強反解釋+為何不成立，或反過度歸因/anti-bias 自檢，守 H7 selfVerify偏誤）"
...
def check_scope(t):
 ...
 return True, "(不涉及停留metric，scope Check不適用)"
    disambig = has(t, r"session\s*級", r"page\s*級", r"user\s*級", r"session-scoped",
                   ...
                   r"session 變深", r"尺度")
 return disambig, "講停留/avgSessionDuration Must區分 session 級 vs page 級（H1 最致命：+334% session 級 ≠ page 級停留）"
...
def check_prereg(t):
 # v1.1：預先註冊紀律 — report有None明說它的假設 + Success/證偽判準（對付 H7 搬龍門）
    ok = has(t, r"假設", r"成功判準", r"證偽判準", r"預先註冊", r"pre.?reg",
             r"hypothesis", r"^windows:", r"成功/?證偽", r"判準")
 return ok, "(v1.1) 宜先declare假設 + Success/證偽判準（pre-registration，對付 H7 事後搬龍門）"


def check_mission(t):
 # v1.1：Goodhart / 使命alignment — 優化型分析有None問「這metric服務 MANIFESTO 還是 goodhart 一代理」
    optimization = has(t, r"改版", r"優化", r"轉換率", r"提升.{0,4}engagement", r"suggestion.{0,8}(上移|重design|加)", r"feature")
    if not optimization:
 return True, "(非優化型分析，使命alignmentCheck不適用)"
    ok = has(t, r"Goodhart", r"使命", r"代理metric", r"proxy", r"MANIFESTO", r"clickbait",
             r"靈魂", r"背叛", r"策展.{0,4}靈魂", r"局部.{0,2}最高")
 return ok, "(v1.1) 優化型分析宜問：這metric服務 MANIFESTO，還是 goodhart 一代理（engagement↑但靈魂↓）"
