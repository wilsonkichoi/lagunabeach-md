#!/usr/bin/env node
/**
 * generate-carousel-slides.mjs — LagunaBeach.md IG carousel slide Generate器
 *
 * 把一份 slide script (JSON) 渲染成 N 1080×1350 (4:5) PNG。
 ...
 *
 * 品牌識別（v0.4，2026-06-03 Cheyu directive「主題色效果不好，先Don't用」）：
 * LagunaBeach.md 主色 #1a3c34 + single accent #00d4aa（識別色）。
 * 不再用category色當secondary accent — 統一識別色，profile feed 一致度更高。
 *
 * Usage:
 ...
 * { "type": "stat", "value": "172→57", "unit": "公里", "label": "25 年來，颱風 24 hourspath預報誤差壓掉三分之二" },
 * { "type": "chart-stat", "title": "一週的代謝", "stats": [{ "value": "310", "label": "次 commit" }, { "value": "28", "label": " PR" }] },
 *       { "type": "chart-bars", "title": "誰在讀台灣的故事？", "bars": [{ "label": "西方 AI", "value": 83, "display": "83%" }, { "label": "對岸", "value": 16, "display": "16%", "note": "成功response率較低" }], "source_note": "Cloudflare 7d" },
 ...
 * { "type": "chart-waffle", "title": "誰拿到", "cells": [{ "label": "軍方", "pct": 48 }, { "label": "民眾", "pct": 37 }], "source_note": "配售structure" },
 ...
 * { "type": "source", "sources": ["中央社","國家太空中心","原視"], "cta": "收藏這・分享給Need的人・Track LagunaBeach.md" }
 *     ]
 *   }
 */
...
if (!scriptPath) {
  console.error('❌ Need --script <path-to-slide-script.json>');
  process.exit(1);
}

// ── brand tokens (LagunaBeach.md 識別) ────────────────────────────────────────────
const BRAND = {
...
// v0.4: 拿掉 accent2，全 deck 統一用 BRAND.accent 識別色（Cheyu directive）
...
<head>
<meta charset="utf-8">
<title>LagunaBeach.md carousel</title>
...
/* section — v0.3: characters級 1.2x、寬度用滿 */
...
/* code excerpt chip — v0.7: section 可選節錄一小段（System名 / metric / 短Output），code-block 風 */
...
/* v0.8: code-block 語法高亮（受控 terminal 配色，Cheyu callout）*/
.codeblock .t-id{color:#5fd3bb;} /* 指令 / 識別characters — soft teal */
.codeblock .t-num{color:#e3b984;} /* 數characters / version — soft amber */
.codeblock .t-op{color:#5f827a;}                 /* operator / tree — dim */
.codeblock .t-cjk{color:rgba(244,240,234,.82);} /* CJK 標籤 — base */
/* section 可選援圖（截圖 / 紀實照）— v0.7：text 主角、圖輔助，跟 figure（圖主角）區隔 */
...
.bcaveat{margin-top:2.2rem;padding-top:1.5rem;border-top:2px solid rgba(0,212,170,.28);font-size:2.04rem;line-height:1.5;color:var(--accent-soft);font-weight:500;} /* v0.8: caveat 常是 punchline，From dim italic 升為 accent + 分隔線 */

/* stat — v0.3: characters級 1.2x、寬度用滿 */
...
/* chart-bars — v0.5: graph.md tw-bars port（水平比例：Sort大→小、長From0、singleaccent、directly標籤＞圖例） */
...
/* chart-stat — v0.5: graph.md tw-stat port（2-4 並排關鍵數characters + 標籤 + 註記） */
...
/* chart-waffle — v0.6: graph.md tw-waffle port（100 格partial對全體；accent 明度階，色盲友善） */
...
/* chart-line — v0.6: graph.md tw-line port（趨勢折線，inline SVG + 終點directly標籤） */
...
/* watermark bottom — v0.3: characters級 1.2x */
...
window.__renderSlide = (s) => {
 ...
 html += '<span class="swipe">滑看full故事 <span class="arrow">→</span></span>';
 ...
 // graph.md tw-bars: Sort大→小（除非 sort:false 固定類別）/ 長From 0 / 寬度比例縮放
 ...
 // graph.md tw-stat: 2-4 並排數characters
 ...
 // graph.md tw-waffle: 100 格partial對全體；accent 明度階（色盲友善）
 // v0.8: 三 tint 明度+彩度雙軸分離，且all明顯亮於主底 #1a3c34（critic：原 dark 類沉進背景數不出來）
 ...
 // graph.md tw-line: 趨勢折線，automatic y 範圍 + 終點directly標籤（不用圖例）
 ...
 // v0.7: 軸線structure + 每點數值標籤（Cheyu callout「缺圖說跟線、看不太懂」）
 ...
 // v0.8: series 可標 ref:true → 虛線基準（dim、無點、無值標籤、single右標）vs 實測序列（critic：平直基準畫成兩點實線像被測量）
 ...
 html += '<div class="srctitle">DataSource　SOURCES</div>';
...
async function main() {
  ...
  // v0.4: auto-open output folder (perCheyu directive 2026-06-03)
