...
# Stats go into: README.md, public/api/stats.json
# (about page stats are data-driven from JSON at build time — this script no
#  longer writes src/i18n/about.ts; see Section 3.)
set -e
cd "$(dirname "$0")/../.."
...
# 3. About page stats: nothing to do here.
# about.template.astro renders totalArticles / starCount / contributorCount
# directly from dashboard-vitals.json + contributors.json at build time
# (data-driven since 2026-06-13). The about.stats.{pages,stars,contributors}.number
# i18n strings this script used to sed are dead and were removed from about.ts, so
# this script no longer touches src/i18n/about.ts at all.
