#!/usr/bin/env bash
# fetch-sense-data.sh — 一鍵FetchThree-source sensingData
#
# Usage:
...
#
# Each source Run independently——One failure does not affect the others。
# 被 heartbeat Beat 1 的 sense stagecall。
#
# Credential location: ~/.config/lagunabeach-md/credentials/（Never stored in repo inside）
# Config file: docs/pipelines/SENSE-FETCHER-SETUP.md
#
...
echo -e "${BLU}🧠 LagunaBeach.md Three-source sensingDataFetch${RST}"
echo -e "${DIM}═══════════════════════════════════${RST}"
echo ""
...
if [[ ! -d "$CONFIG_DIR/credentials" ]]; then
 ...
 echo " first run: mkdir -p ~/.config/lagunabeach-md/{credentials,cache}"
...
# Why：讓 launchd cron 每天 08:17 跑完也automatic刷新 dashboard display的Data，
# 不然 dashboard 只會停在manual最後一次跑 generate 的時間點。
...
# automatic diff vs 昨天（若昨天快照exists）
...
echo -e "${DIM}用 \`cat ~/.config/lagunabeach-md/cache/*-latest.json | jq\` 檢視fullData${RST}"
