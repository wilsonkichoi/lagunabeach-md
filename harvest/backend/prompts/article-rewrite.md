...
You are taking on a LagunaBeach.md article task. Use the canonical pipeline document `docs/pipelines/REWRITE-PIPELINE.md` as your SOP — your boot profile already loaded it.
...
2. **Stage 1 research**: write to `reports/research/YYYY-MM/{slug}.md`. Follow `docs/editorial/RESEARCH-TEMPLATE.md` and the WebFetch verbatim rule from RESEARCH.md §6.
3. **Stage 2 draft**: full read of `docs/editorial/EDITORIAL.md` is mandatory before drafting (don't trust memory).
4. **Stage 3 fact triangle self-check**: arithmetic, units, direct quotes need 3x verification (per the observer's repeated guidance).
...
9. **Stage 5 commit**: pre-commit hook is the final gate. **If the pre-commit hook fails, do NOT bypass with `--no-verify`. Fix the underlying issue and commit again.** Bypassing the hook is a slacking marker the harvest engine actively detects.
...
- **Memory log** (mandatory, after final commit): append a Beat-5-style entry to `docs/semiont/memory/YYYY-MM-DD-harvest-{{task.id}}.md` capturing:
  ...
  - wall-clock duration from `git log %ai` of your commits (NOT subjective time sense — MANIFESTO §time is structure)
  - signature 🧬 + your harvest session id
