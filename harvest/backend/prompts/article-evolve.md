...
This is an EVOLVE pass on an existing article. Walk REWRITE-PIPELINE Stage 0 (material extraction) before normal stages 1–6.
...
8. **Stage 5 commit**: pre-commit hook is the final gate. **If it fails, do NOT bypass with `--no-verify`. Fix the underlying issue and commit again.** Bypassing the hook is a slacking marker the harvest engine actively detects.
