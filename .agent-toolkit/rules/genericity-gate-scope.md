...
- **`check-english-only.mjs`** — five roots: `src/`, `scripts/`, `tests/`, `workers/`,
  `.agent/skills/`. `workers/` is in the CJK gate's roots alone, so in instance mode a
...
`.agent/skills/` with the framework skills in 5.6. Test fixtures are code and so is
